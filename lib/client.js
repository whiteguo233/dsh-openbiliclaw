window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region lib/types/client/drag.js
		/**
		...
		*/
		...
		const PANEL_COL_SELECTOR = "[data-obc-panel-col]";
		...
		var PanelLayoutController = class {
			...
			applyGrid() {
				...
				if (this.column !== null) this.column.style.visibility = width > 0 ? "visible" : "hidden";
				if (this.handle !== null) {
			...
			createHandle() {
				...
				el.addEventListener("pointerdown", (event) => {
					handlePointerDragStart(event, el, {
						...
						onFrame: (width) => {
							this.store.update((prev) => prev.width === width ? prev : {
								...prev,
								width
							});
						},
				...
				el.addEventListener("dblclick", () => {
					this.store.update((prev) => prev.width === 380 ? prev : {
						...prev,
						width: 380
					});
					persistPanelWidth(380);
					this.applyGrid();
				});
				return el;
			}
			...
			clampWidth(requested) {
				...
				const maxByContainer = Math.max(280, available - 400);
				return Math.min(560, Math.min(requested, maxByContainer));
			}
			/**
			...
			*/
			...
			setOpen(open) {
				this.store.update((prev) => prev.open === open ? prev : {
					...prev,
					open
				});
		...
		*/
		function summarizeDetail(detail) {
			let value = detail;
			if (typeof value === "object" && value !== null && "detail" in value) value = value.detail;
			...
			if (Array.isArray(value)) {
				...
				for (const entry of value) {
					...
					if (msg === "") continue;
					const loc = Array.isArray(rec.loc) ? rec.loc.filter((seg) => typeof seg === "string").join(".") : "";
		...
		*  (e.g. `&gt;&gt;&gt;`); `&amp;` is decoded last so double-escaped text unwraps once. */
		...
		async function sendBehaviorEvents(base, events, signal) {
			...
			return {
				...
				rejected: Array.isArray(row.rejected) ? row.rejected.map((r) => ({
					index: num(r.index),
					type: str(r.type),
					reason: str(r.reason)
				})) : []
		...
		*  Returns the settlement response (state/verdict are authoritative). */
		...
		async function fetchSaved(base, listKind, signal) {
			const data = await requestJson(base, `/api/saved/${listKind}`, {
				timeoutMs: 1e4,
				signal
			});
			const row = typeof data === "object" && data !== null ? data : {};
			return {
				items: asItems(data).map(toSaved),
				total: num(row.total)
			};
		}
		...
		*  An empty item_keys array means all eligible rows for this list. */
		async function syncSavedItems(base, listKind, itemKeys = [], signal) {
			return toSyncBatch(await requestJson(base, `/api/saved/${listKind}/sync`, {
				method: "POST",
		...
		async function pollSavedSyncTask(base, taskId, signal) {
			return toSyncBatch(await requestJson(base, `/api/saved-sync/tasks/${encodeURIComponent(taskId.trim())}`, {
				timeoutMs: 15e3,
				signal
			}));
		}
		...
		*  the backend persists + hot-reloads. */
		...
		async function fetchProfileSummary(base, opts = {}) {
			try {
				const params = new URLSearchParams();
				params.set("limit", String(opts.limit ?? 5));
				if (opts.cursor !== void 0 && opts.cursor !== "") params.set("cursor", opts.cursor);
				const data = await requestJson(base, `/api/profile-summary?${params.toString()}`, {
					timeoutMs: 1e4,
					signal: opts.signal
				});
				if (typeof data !== "object" || data === null) return null;
				...
				return {
					...
					recent_cognition_updates: Array.isArray(row.recent_cognition_updates) ? row.recent_cognition_updates.map((c) => ({
		...
		async function fetchProjectStats(base, signal) {
			try {
				const data = await requestJson(base, "/api/project-stats", {
					timeoutMs: 6e3,
					signal
				});
				const n = Number((typeof data === "object" && data !== null ? data : {}).github_stars);
		...
		*/
		...
		function MonitorIcon({ size = 15 }) {
			return (0, react_jsx_runtime.jsxs)("svg", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("path", { d: "M9 20.5h6" }),
					(0, react_jsx_runtime.jsx)("path", { d: "M12 17v3.5" })
				]
			});
		}
		/** GitHub star mark — the popup's gh-star-mark (filled). */
		function GithubStarIcon({ size = 14 }) {
			return (0, react_jsx_runtime.jsx)("svg", {
				width: size,
				height: size,
				viewBox: "0 0 16 16",
				fill: "currentColor",
				"aria-hidden": "true",
				children: (0, react_jsx_runtime.jsx)("path", { d: "M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.02-1.49-2.22.48-2.69-1.07-2.69-1.07-.36-.92-.89-1.17-.89-1.17-.73-.5.05-.49.05-.49.81.06 1.23.83 1.23.83.72 1.23 1.89.88 2.35.67.07-.52.28-.88.51-1.08-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82a7.6 7.6 0 0 1 2-.27c.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8z" })
			});
		}
		//#endregion
		//#region \0dsh-css:/Users/white/workspace/OpenBiliClaw/.worktrees/dsh-plugin-parity/src/client/panel.module.css.mjs
		const css = ".PA7OrW_panel{--obc-bg:#fffafc;--obc-sky:#f2f8ff;--obc-surface:#ffffffd6;--obc-surface-strong:#fffffff7;--obc-surface-soft:#ffffffb8;--obc-line:#d9e3f2f2;--obc-line-strong:#f7adca80;--obc-text:#20304a;--obc-text-secondary:#60708c;--obc-text-muted:#8290a8;--obc-brand:#fb7299;--obc-brand-strong:#f65788;--obc-brand-soft:#fb72991f;--obc-sky-blue:#5aa9ff;--obc-sky-soft:#5aa9ff24;--obc-success:#30b980;--obc-danger:#ef7a86;--obc-shadow-lg:0 20px 40px #384c701f;--obc-shadow-sm:0 10px 22px #495d8214;min-width:0;height:100%;color:var(--obc-text);background:radial-gradient(circle at 15% 8%, #fb729929, transparent 30%), radial-gradient(circle at 88% 12%, #5aa9ff2e, transparent 26%), linear-gradient(180deg, var(--obc-bg) 0%, var(--obc-sky) 100%);background-attachment:fixed;flex-direction:column;font-family:Avenir Next,PingFang SC,Hiragino Sans GB,Microsoft YaHei,sans-serif;display:flex;position:relative;overflow:hidden}.PA7OrW_panel button,.PA7OrW_panel input,.PA7OrW_panel textarea{font-family:inherit}.PA7OrW_header{flex:none;align-items:center;gap:6px;padding:14px 14px 10px;display:flex}.PA7OrW_brand{flex:1;align-items:center;gap:9px;min-width:0;display:flex}.PA7OrW_brandMark{object-fit:cover;background:linear-gradient(135deg, var(--obc-brand), #ff8cb0);border-radius:10px;flex:none;width:30px;height:30px;display:block;box-shadow:0 8px 16px #fb729940}.PA7OrW_brandCopy{flex-direction:column;min-width:0;display:flex}.PA7OrW_brandTitle{color:var(--obc-text);letter-spacing:-.02em;white-space:nowrap;text-overflow:ellipsis;font-size:14px;font-weight:800;overflow:hidden}.PA7OrW_status{align-items:center;gap:4px;display:flex}.PA7OrW_statusDot{background:var(--obc-danger);border-radius:50%;width:6px;height:6px}.PA7OrW_statusDot[data-online=true]{background:var(--obc-success)}.PA7OrW_statusText{color:var(--obc-text-muted);font-size:10px}.PA7OrW_iconButton{width:30px;height:30px;color:var(--obc-text-secondary);cursor:pointer;background:#ffffffc2;border:1px solid #e6edf8eb;border-radius:10px;flex:none;justify-content:center;align-items:center;transition:background .15s,transform .15s;display:flex;position:relative}.PA7OrW_starButton{height:30px;color:var(--obc-brand-strong);cursor:pointer;background:linear-gradient(#fffcfef5,#fb72991a);border:1px solid #fb729947;border-radius:10px;flex:none;align-items:center;gap:5px;padding:0 9px;transition:transform .15s,border-color .15s,box-shadow .15s;display:inline-flex}.PA7OrW_starButton:hover{border-color:#fb72998c;transform:translateY(-1px);box-shadow:0 6px 14px #fb72992e}.PA7OrW_starCount{color:var(--obc-text-secondary);font-size:10.5px;font-weight:800;line-height:1}.PA7OrW_starLabel{color:var(--obc-brand-strong);font-size:10.5px;font-weight:800;line-height:1}.PA7OrW_iconButton:hover{background:var(--obc-brand-soft);color:var(--obc-brand-strong);transform:translateY(-1px)}.PA7OrW_bellBadge{background:var(--obc-brand);color:#fff;min-width:16px;height:16px;box-shadow:0 0 0 2px var(--obc-bg);border-radius:8px;justify-content:center;align-items:center;padding:0 4px;font-size:9px;font-weight:700;display:flex;position:absolute;top:-5px;right:-5px}.PA7OrW_settings{border-bottom:1px solid var(--obc-line);flex-direction:column;gap:6px;padding:0 14px 10px;display:flex}.PA7OrW_settingsRow{align-items:center;gap:6px;display:flex}.PA7OrW_settingsInput{min-width:0;color:var(--obc-text);background:#ffffffd9;border:1px solid #e6edf8eb;border-radius:8px;flex:1;padding:6px 9px;font-size:11px}.PA7OrW_hint{color:var(--obc-text-muted);font-size:10.5px;line-height:1.5}.PA7OrW_tabBar{flex:none;gap:6px;padding:0 14px 12px;display:flex}.PA7OrW_tab{min-width:0;min-height:48px;color:var(--obc-text-secondary);cursor:pointer;background:0 0;border:0;border-radius:16px;flex-direction:column;flex:1;justify-content:center;align-items:center;gap:3px;padding:7px 4px;font-size:10px;font-weight:700;line-height:1.3;transition:background .18s,color .18s,box-shadow .18s,transform .18s;display:flex}.PA7OrW_tab:hover{color:var(--obc-text);background:#ffffff85}.PA7OrW_tab[data-active=true]{color:var(--obc-brand-strong);background:linear-gradient(#fffffffa,#fff0f6fa);transform:translateY(-1px);box-shadow:0 10px 18px #fb729924}.PA7OrW_badge{background:var(--obc-brand);color:#fff;text-align:center;vertical-align:1px;border-radius:8px;min-width:15px;height:15px;margin-left:3px;padding:0 4px;font-size:9px;font-weight:700;line-height:15px;display:inline-block}.PA7OrW_body{flex-direction:column;flex:1;gap:12px;min-height:0;padding:2px 14px 20px;display:flex;overflow-y:auto}.PA7OrW_body>*{flex-shrink:0}.PA7OrW_empty{color:var(--obc-text-muted);text-align:center;padding:26px 10px;font-size:12px;line-height:1.7}.PA7OrW_error{color:var(--obc-danger);white-space:pre-wrap;background:#ef7a861a;border:1px solid #ef7a8640;border-radius:10px;padding:8px 10px;font-size:11.5px;line-height:1.5}.PA7OrW_toolbar{flex:none;align-items:center;gap:6px;display:flex}.PA7OrW_spacer{flex:1}.PA7OrW_sectionTitle{color:var(--obc-text);margin:4px 0 2px;font-size:12px;font-weight:800}.PA7OrW_recHeader{box-shadow:var(--obc-shadow-sm);background:linear-gradient(#ffffffe6,#fafbffe6);border:1px solid #e6edf8eb;border-radius:18px;align-items:flex-start;gap:8px;padding:14px;display:flex}.PA7OrW_recHeaderCopy{flex:1;min-width:0}.PA7OrW_recHeaderActions{flex-direction:column;align-items:stretch;gap:6px;display:flex}.PA7OrW_initPrompt{box-shadow:var(--obc-shadow-sm);background:linear-gradient(#ffffffe6,#fafbffe6);border:1px solid #e6edf8eb;border-radius:16px;align-items:center;gap:10px;padding:12px 14px;display:flex}.PA7OrW_initPromptCopy{min-width:0;color:var(--obc-text-secondary);flex-direction:column;flex:1;gap:4px;font-size:11px;line-height:1.55;display:flex}.PA7OrW_initPromptCopy strong{color:var(--obc-text);font-size:13px}.PA7OrW_initPromptStatus{color:var(--obc-text-muted)}.PA7OrW_recKicker{letter-spacing:.08em;text-transform:uppercase;color:var(--obc-brand);font-size:9px;font-weight:700}.PA7OrW_recHeaderTitle{color:var(--obc-text);letter-spacing:-.02em;margin-top:2px;font-size:16px;font-weight:800;line-height:1.35}.PA7OrW_poolChips{flex-direction:column;gap:4px;margin-top:8px;display:flex}.PA7OrW_poolChip{color:var(--obc-text-secondary);background:#f0f6ffe6;border-radius:6px;justify-content:space-between;gap:8px;padding:3px 7px;font-size:10px;line-height:1.5;display:flex}.PA7OrW_poolChipLabel{color:var(--obc-text-muted);flex:none;font-weight:600}.PA7OrW_poolChipValue{text-align:right;word-break:break-word}.PA7OrW_delightCard{background:linear-gradient(#fffcfefa,#fcf8fffa);border:1px solid #fb729947;border-radius:18px;flex-direction:column;gap:6px;padding:12px;display:flex;box-shadow:inset 0 1px #ffffffd1,0 4px 14px #fb72991a}.PA7OrW_delightKickerLine{align-items:center;gap:6px;display:flex}.PA7OrW_delightKicker{color:var(--obc-brand-strong);font-size:12px;font-weight:800}.PA7OrW_delightCounter{color:var(--obc-text-muted);margin-left:auto;font-size:10px}.PA7OrW_delightTitle{color:var(--obc-text);letter-spacing:-.01em;-webkit-line-clamp:2;word-break:break-word;-webkit-box-orient:vertical;font-size:14px;font-weight:800;line-height:1.4;display:-webkit-box;overflow:hidden}.PA7OrW_card{box-shadow:var(--obc-shadow-sm);background:linear-gradient(#ffffffeb,#fafbffeb);border:1px solid #e6edf8eb;border-radius:16px;align-items:flex-start;gap:10px;padding:10px;transition:border-color .18s,box-shadow .18s;display:flex}.PA7OrW_card:hover{border-color:var(--obc-line-strong);box-shadow:0 14px 26px #fb72991a}.PA7OrW_thumb{object-fit:cover;background:linear-gradient(145deg,#fb72991f,#5aa9ff1a);border-radius:10px;flex:none;width:84px;height:58px;display:block}.PA7OrW_thumbFallback{width:84px;height:58px;color:var(--obc-text-secondary);background:linear-gradient(145deg,#fb72991f,#5aa9ff1a);border-radius:10px;flex:none;justify-content:center;align-items:center;font-size:16px;font-weight:700;display:flex}.PA7OrW_coverWrap{flex:none;position:relative}.PA7OrW_coverCorner{color:#fff;background:#20304a9e;border-radius:4px;padding:0 4px;font-size:8px;font-weight:700;line-height:13px;position:absolute;top:4px;left:4px}.PA7OrW_cardBody{flex-direction:column;flex:1;align-self:center;gap:3px;min-width:0;display:flex}.PA7OrW_cardTitle{min-height:36px;color:var(--obc-text);letter-spacing:-.01em;-webkit-line-clamp:2;word-break:break-word;-webkit-box-orient:vertical;font-size:13px;font-weight:800;line-height:1.4;display:-webkit-box;overflow:hidden}.PA7OrW_cardMeta{color:var(--obc-text-muted);flex-wrap:wrap;align-items:center;gap:6px;font-size:10px;display:flex}.PA7OrW_platformTag{color:var(--obc-text-secondary);background:#f0f6ffe6;border-radius:4px;padding:0 5px;font-size:9px;font-weight:600;line-height:15px}.PA7OrW_badgeRow{flex-wrap:wrap;align-items:center;gap:5px;display:flex}.PA7OrW_stateBadge{color:var(--obc-text-muted);background:#f0f6ffe6;border-radius:5px;padding:0 5px;font-size:9px;line-height:15px}.PA7OrW_topicBadge{background:var(--obc-brand-soft);color:var(--obc-brand-strong);border-radius:5px;padding:0 5px;font-size:9px;font-weight:700;line-height:15px}.PA7OrW_contextBadge{color:var(--obc-text-secondary);background:#f0f6ffe6;border-radius:4px;padding:0 5px;font-size:9px;font-weight:600;line-height:15px}.PA7OrW_contextBadge[data-kind=restored]{color:var(--obc-success);background:#30b9801f}.PA7OrW_contextBadge[data-kind=removed]{color:var(--obc-danger);background:#ef7a861f}.PA7OrW_stats{color:var(--obc-text-muted);flex-wrap:wrap;gap:8px;font-size:10px;display:flex}.PA7OrW_expression{color:var(--obc-text-secondary);-webkit-line-clamp:3;-webkit-box-orient:vertical;font-size:11.5px;line-height:1.6;display:-webkit-box;overflow:hidden}.PA7OrW_feedbackStatus{color:var(--obc-text-muted);min-height:13px;font-size:10.5px;line-height:1.4}.PA7OrW_cardActions{flex-wrap:wrap;gap:6px;margin-top:3px;display:flex}.PA7OrW_actionButton{cursor:pointer;min-height:26px;color:var(--obc-text-secondary);background:#f0f6ffe6;border:0;border-radius:11px;padding:4px 10px;font-size:11px;font-weight:600;line-height:1.2;transition:background .18s,color .18s,box-shadow .18s,transform .18s}.PA7OrW_actionButton:hover:not(:disabled){color:var(--obc-text);transform:translateY(-1px)}.PA7OrW_actionButton:disabled{cursor:not-allowed;opacity:.6;transform:none}.PA7OrW_actionButton[data-primary=true]{background:linear-gradient(135deg, var(--obc-brand), #ff8cb0);color:#fff;font-weight:800;box-shadow:0 8px 14px #fb729938}.PA7OrW_actionButton[data-primary=true]:hover:not(:disabled){background:linear-gradient(135deg, var(--obc-brand-strong), #ff7ca7);color:#fff}.PA7OrW_actionButton[data-danger=true]:hover:not(:disabled){color:var(--obc-danger);background:#ef7a861f}.PA7OrW_commentRow{align-items:center;gap:6px;display:flex}.PA7OrW_commentInput{min-width:0;color:var(--obc-text);background:#ffffffd9;border:1px solid #e6edf8eb;border-radius:8px;flex:1;padding:4px 8px;font-size:11px}.PA7OrW_commentInput:focus{border-color:var(--obc-sky-blue);outline:none}.PA7OrW_loadMore{color:var(--obc-text-secondary);cursor:pointer;background:0 0;border:1px dashed #d9e3f2f2;border-radius:10px;padding:7px;font-size:11.5px}.PA7OrW_loadMore:hover:not(:disabled){color:var(--obc-text);border-color:var(--obc-sky-blue)}.PA7OrW_loadMore:disabled{opacity:.5}.PA7OrW_loadingRow{color:var(--obc-text-muted);justify-content:center;align-items:center;gap:8px;padding:10px 0 4px;font-size:11px;font-weight:700;display:flex}.PA7OrW_spinner{border:2px solid #fb729938;border-top-color:var(--obc-brand);border-radius:999px;width:14px;height:14px;animation:.8s linear infinite PA7OrW_obcSpin}@keyframes PA7OrW_obcSpin{to{transform:rotate(360deg)}}.PA7OrW_activityFooter{border-top:1px solid var(--obc-line);padding-top:8px}.PA7OrW_activityRow{color:var(--obc-text-secondary);gap:8px;padding:4px 0;font-size:11px;line-height:1.5;display:flex}.PA7OrW_activityTime{color:var(--obc-text-muted);flex:none;padding-top:1px;font-size:10px}.PA7OrW_subTabs{border:1px solid var(--obc-line);background:#ffffffa8;border-radius:12px;flex:none;gap:4px;padding:4px;display:flex}.PA7OrW_subTab{color:var(--obc-text-secondary);cursor:pointer;white-space:nowrap;background:0 0;border:0;border-radius:9px;flex:1;padding:6px 4px;font-size:10.5px;font-weight:700;line-height:1.4}.PA7OrW_subTab:hover{color:var(--obc-text)}.PA7OrW_subTab[data-active=true]{background:var(--obc-surface-strong);color:var(--obc-brand-strong);box-shadow:var(--obc-shadow-sm)}.PA7OrW_syncToolbar{flex-direction:column;align-items:flex-start;gap:5px;margin-bottom:10px;display:flex}.PA7OrW_syncMessage{color:var(--obc-text-muted);min-height:13px;font-size:10.5px;line-height:1.45}.PA7OrW_syncMessage[data-tone=error]{color:var(--obc-danger)}.PA7OrW_syncResults{border:1px solid var(--obc-line);background:#fff9;border-radius:12px;flex-direction:column;gap:5px;max-height:140px;margin-bottom:10px;padding:7px 8px;display:flex;overflow-y:auto}.PA7OrW_syncLine{min-width:0;color:var(--obc-text-secondary);align-items:center;gap:6px;font-size:10.5px;line-height:1.45;display:flex}.PA7OrW_syncLineText{text-overflow:ellipsis;white-space:nowrap;color:var(--obc-text);flex:0 auto;font-weight:700;overflow:hidden}.PA7OrW_syncLineDetail{text-overflow:ellipsis;white-space:nowrap;color:var(--obc-text-muted);flex:1;overflow:hidden}.PA7OrW_syncBadgeRow{align-items:center;gap:6px;min-width:0;display:flex}.PA7OrW_syncInlineDetail{text-overflow:ellipsis;white-space:nowrap;color:var(--obc-text-muted);font-size:10px;overflow:hidden}.PA7OrW_syncChip{color:var(--obc-text-secondary);background:#f0f6ffe6;border-radius:999px;flex:none;padding:2px 7px;font-size:9.5px;font-weight:700;line-height:15px}.PA7OrW_syncChip[data-tone=success]{color:var(--obc-success);background:#30b9801f}.PA7OrW_syncChip[data-tone=warning]{color:#8a6520;background:#ffc10724}.PA7OrW_syncChip[data-tone=error]{color:var(--obc-danger);background:#ef7a861f}.PA7OrW_syncChip[data-tone=info]{color:var(--obc-brand-strong);background:var(--obc-brand-soft)}.PA7OrW_savedFeedbackRow{flex-wrap:wrap;gap:5px;margin-top:3px;display:flex}.PA7OrW_historyRestoreList{flex-direction:column;gap:5px;margin-top:3px;display:flex}.PA7OrW_historyRestoreRow{align-items:center;gap:6px;display:flex}.PA7OrW_historyRestoreLabel{color:var(--obc-text-muted);text-overflow:ellipsis;white-space:nowrap;font-size:10px;overflow:hidden}.PA7OrW_turn{flex-direction:column;gap:3px;display:flex}.PA7OrW_turnUser,.PA7OrW_turnSoul{white-space:pre-wrap;word-break:break-word;border-radius:14px;padding:8px 11px;font-size:12px;line-height:1.6}.PA7OrW_turnUser{background:var(--obc-brand-soft);color:var(--obc-text);align-self:flex-end;max-width:92%}.PA7OrW_turnSoul{border:1px solid var(--obc-line);color:var(--obc-text);background:#ffffffe6;align-self:flex-start;max-width:92%}.PA7OrW_turnStatus{color:var(--obc-text-muted);align-self:flex-end;font-size:10px}.PA7OrW_chatInputRow{border-top:1px solid var(--obc-line);flex:none;gap:6px;padding-top:8px;display:flex}.PA7OrW_chatInput{min-width:0;color:var(--obc-text);resize:none;background:#ffffffe6;border:1px solid #e6edf8eb;border-radius:12px;flex:1;padding:8px 10px;font-size:12px}.PA7OrW_chatInput:focus{border-color:var(--obc-sky-blue);outline:none}.PA7OrW_chatSend{background:linear-gradient(135deg, var(--obc-brand), #ff8cb0);color:#fff;cursor:pointer;border:0;border-radius:12px;padding:0 14px;font-size:12px;font-weight:800;box-shadow:0 8px 14px #fb729938}.PA7OrW_chatSend:hover:not(:disabled){background:linear-gradient(135deg, var(--obc-brand-strong), #ff7ca7)}.PA7OrW_chatSend:disabled{opacity:.6}.PA7OrW_confirmPanel{border:1px solid var(--obc-line);background:#ffffffd9;border-radius:14px;flex:none;overflow:hidden}.PA7OrW_confirmToggle{width:100%;color:var(--obc-text);cursor:pointer;text-align:left;background:0 0;border:none;align-items:center;gap:6px;padding:9px 11px;font-size:12px;font-weight:800;display:flex}.PA7OrW_confirmToggle:hover{background:var(--obc-brand-soft)}.PA7OrW_confirmCount{color:#fff;background:var(--obc-brand);border-radius:8px;margin-left:auto;padding:1px 7px;font-size:10px;font-weight:700}.PA7OrW_confirmItem{border-top:1px solid var(--obc-line);flex-direction:column;gap:4px;padding:10px 11px;display:flex}.PA7OrW_confirmTitle{color:var(--obc-text);font-size:12px;font-weight:800;line-height:1.5}.PA7OrW_cardTurn{border:1px solid var(--obc-line);background:#ffffffe6;border-radius:14px;flex-direction:column;gap:4px;padding:10px 11px;display:flex}.PA7OrW_insightEvidence{color:var(--obc-text-secondary);white-space:pre-wrap;font-size:11px;line-height:1.55}.PA7OrW_insightNote{color:var(--obc-text-muted);font-size:10.5px}.PA7OrW_probeConfidence{color:var(--obc-text-muted);font-size:10px}.PA7OrW_portrait{color:var(--obc-text);border:1px solid var(--obc-line);background:#ffffffd9;border-radius:14px;padding:11px 12px;font-size:12.5px;line-height:1.7}.PA7OrW_chipRow{flex-wrap:wrap;gap:5px;display:flex}.PA7OrW_chip{color:var(--obc-text-secondary);background:#f0f6ffe6;border-radius:11px;padding:2px 8px;font-size:10.5px;font-weight:600;line-height:1.6}.PA7OrW_chip[data-kind=dislike]{color:var(--obc-danger);background:#ef7a861f}.PA7OrW_interestDomain{margin-bottom:6px}.PA7OrW_interestDomainHead{color:var(--obc-text);align-items:center;gap:6px;font-size:12px;font-weight:800;display:flex}.PA7OrW_domainWeight{color:var(--obc-text-muted);font-size:10px;font-weight:600}.PA7OrW_awarenessRow{color:var(--obc-text-secondary);padding:4px 0;font-size:11px;line-height:1.55}.PA7OrW_awarenessDate{color:var(--obc-text-muted);margin-right:6px}.PA7OrW_probe{border:1px solid var(--obc-line);background:#ffffffd9;border-radius:12px;flex-direction:column;gap:4px;padding:10px 11px;display:flex}.PA7OrW_probeDomain{color:var(--obc-text);font-size:12px;font-weight:800}.PA7OrW_probeReason{color:var(--obc-text-secondary);font-size:11.5px;line-height:1.55}.PA7OrW_probeActions{flex-wrap:wrap;gap:6px;margin-top:2px;display:flex}.PA7OrW_drawerOverlay{z-index:10;backdrop-filter:blur(2px);background:#20304a47;flex-direction:column;justify-content:flex-start;display:flex;position:absolute;inset:0}.PA7OrW_drawerPanel{background:radial-gradient(circle at 15% 4%, #fb729924, transparent 30%), var(--obc-bg);border-radius:18px 18px 0 0;flex-direction:column;flex:1;gap:10px;min-height:0;margin-top:8px;padding:14px 14px 24px;display:flex;overflow-y:auto}.PA7OrW_drawerHeader{border-bottom:1px solid var(--obc-line);justify-content:space-between;align-items:center;padding-bottom:10px;display:flex}.PA7OrW_drawerTitle{color:var(--obc-text);align-items:center;gap:6px;font-size:15px;font-weight:800;display:flex}.PA7OrW_messageCard{background:var(--obc-surface-strong);border:1px solid var(--obc-line);border-radius:14px;flex-direction:column;gap:5px;padding:11px 12px;display:flex}.PA7OrW_messageCard[data-tone=interest]{background:linear-gradient(135deg,#fff8e8,#fff0d5);border-color:#dd7f2d47}.PA7OrW_messageCard[data-tone=avoidance]{background:linear-gradient(135deg,#eef7ff,#e1effc);border-color:#3b82f647}.PA7OrW_messageCard[data-tone=challenge]{background:linear-gradient(135deg,#f7f2ff,#ede9fe);border-color:#7c3aed47}.PA7OrW_messageCard[data-tone=delight]{background:linear-gradient(135deg,#fff5f8,#ffe9f0);border-color:#fb729952}.PA7OrW_messageType{color:var(--obc-text-secondary);align-items:center;gap:5px;font-size:10.5px;font-weight:800;display:flex}.PA7OrW_messageCard[data-tone=interest] .PA7OrW_messageType{color:#c26a1d}.PA7OrW_messageCard[data-tone=avoidance] .PA7OrW_messageType{color:#2b6cc4}.PA7OrW_messageCard[data-tone=challenge] .PA7OrW_messageType{color:#7c3aed}.PA7OrW_messageCard[data-tone=delight] .PA7OrW_messageType{color:var(--obc-brand-strong)}.PA7OrW_messagePrompt{color:var(--obc-text-secondary);font-size:10.5px;line-height:1.5}.PA7OrW_messageTitle{color:var(--obc-text);font-size:13px;font-weight:800;line-height:1.45}.PA7OrW_messageBody{color:#55657f;-webkit-line-clamp:4;-webkit-box-orient:vertical;font-size:11px;line-height:1.55;display:-webkit-box;overflow:hidden}.PA7OrW_messageActions{flex-wrap:wrap;gap:6px;margin-top:2px;display:flex}.PA7OrW_drawerEmpty{color:var(--obc-text-muted);flex-direction:column;align-items:center;gap:8px;padding:34px 20px;display:flex}.PA7OrW_drawerEmptyTitle{font-size:13px;font-weight:700}.PA7OrW_drawerEmptySubtitle{opacity:.75;font-size:11px}.PA7OrW_recCard{background:linear-gradient(#fffffffa,#fafbfffa);border:1px solid #ebf0f8fa;border-radius:18px;flex-direction:column;gap:9px;padding:10px;transition:border-color .18s,box-shadow .18s;display:flex;box-shadow:inset 0 1px #ffffffd1}.PA7OrW_recCard:hover{border-color:var(--obc-line-strong);box-shadow:0 18px 32px #fb729924}.PA7OrW_recCover{aspect-ratio:16/9;cursor:pointer;width:100%;color:var(--obc-text-secondary);text-align:center;background:linear-gradient(145deg,#fb72991f,#5aa9ff1a),#f6f9fff0;border:0;border-radius:13px;justify-content:center;align-items:center;padding:0;font-size:12px;font-weight:700;line-height:1.5;display:flex;position:relative;overflow:hidden}.PA7OrW_recCover img{object-fit:cover;width:100%;height:100%;display:block}.PA7OrW_recCoverText{color:var(--obc-text);-webkit-line-clamp:4;-webkit-box-orient:vertical;padding:12px;font-size:12px;line-height:1.6;display:-webkit-box;overflow:hidden}.PA7OrW_recCover .PA7OrW_coverCorner{top:8px;left:8px}.PA7OrW_recBody{flex-direction:column;gap:4px;padding:0 2px;display:flex}.PA7OrW_recTitle{color:var(--obc-text);letter-spacing:-.01em;-webkit-line-clamp:2;word-break:break-word;-webkit-box-orient:vertical;font-size:15px;font-weight:800;line-height:1.38;display:-webkit-box;overflow:hidden}.PA7OrW_delightRow{align-items:stretch;gap:10px;display:flex}.PA7OrW_delightThumb{object-fit:cover;border-radius:12px;flex:none;align-self:center;width:96px;height:96px;box-shadow:inset 0 1px #ffffffb8}.PA7OrW_delightThumbFallback{box-sizing:border-box;width:96px;height:96px;color:var(--obc-text-secondary);text-align:center;background:linear-gradient(145deg,#fb729924,#5aa9ff1f);border-radius:12px;flex:none;justify-content:center;align-self:center;align-items:center;padding:8px;font-size:10.5px;font-weight:700;line-height:1.5;display:flex;overflow:hidden}.PA7OrW_delightCard{background:radial-gradient(circle at 100% 0,#fb729924,#0000 38%),linear-gradient(#fffcfefa,#f6fafffa);border:1px solid #fb729947;border-radius:18px;flex-shrink:0;grid-template-columns:minmax(0,1fr) 40px;gap:0;padding:0;transition:border-color .2s,box-shadow .2s;display:grid;overflow:hidden;box-shadow:inset 0 1px #ffffffd1,0 4px 14px #fb72991a}.PA7OrW_delightRow{cursor:pointer;text-align:left;min-width:0;color:inherit;background:0 0;border:0;grid-area:1/1;align-items:center;gap:10px;padding:10px;display:flex}.PA7OrW_delightThumb{aspect-ratio:16/9;object-fit:cover;background:linear-gradient(#fffffffa,#fff8fcf0),linear-gradient(145deg,#fb72992e,#a855f71f);border:1px solid #ffffffeb;border-radius:12px;flex:none;width:104px;height:auto;transition:transform .2s,box-shadow .2s;box-shadow:inset 0 1px #ffffffb8,0 2px 8px #fb72991f}.PA7OrW_delightCard:hover .PA7OrW_delightThumb{transform:scale(1.02);box-shadow:inset 0 1px #ffffffb8,0 6px 14px #fb729938}.PA7OrW_delightText{flex-direction:column;flex:1;gap:6px;min-width:0;display:flex}.PA7OrW_delightKickerLine{flex-wrap:wrap;align-items:center;gap:5px;min-width:0;display:flex}.PA7OrW_delightKicker{color:var(--obc-brand-strong);letter-spacing:.02em;text-overflow:ellipsis;white-space:nowrap;background:#fb72991a;border-radius:999px;max-width:100%;padding:2px 7px;font-size:9.5px;font-weight:700;overflow:hidden}.PA7OrW_delightCounter{color:var(--obc-text-muted);white-space:nowrap;background:#0000000a;border-radius:8px;flex:none;padding:1px 6px;font-size:10px;font-weight:600}.PA7OrW_delightNav:active:not(:disabled){transform:scale(.94)}.PA7OrW_delightDismiss{color:var(--obc-text-muted);cursor:pointer;background:0 0;border:0;border-left:1px solid #eddbe999;grid-area:1/2;font-size:15px;line-height:1;transition:background .15s,color .15s}.PA7OrW_delightBody{border-top:1px solid #eddbe999;flex-direction:column;grid-area:2/1/auto/-1;gap:8px;padding:4px 12px 12px;display:flex}.PA7OrW_activityFooter{border:1px solid var(--obc-line);backdrop-filter:blur(18px);box-shadow:var(--obc-shadow-sm);background:#ffffffa8;border-radius:14px;flex-direction:column;gap:8px;padding:10px 12px;display:flex}.PA7OrW_activityLine{color:var(--obc-text-secondary);font-size:11.5px;line-height:1.5}.PA7OrW_footerItem{background:#fffc;border:1px solid #dfe7f3f5;border-radius:12px;flex-direction:column;gap:3px;padding:8px 10px;display:flex}.PA7OrW_footerItemMeta{color:var(--obc-text-muted);flex-wrap:wrap;align-items:center;gap:7px;font-size:9.5px;font-weight:700;line-height:1.4;display:flex}.PA7OrW_footerItemKind{min-height:18px;color:var(--obc-text-secondary);background:#fffffff5;border:1px solid #dfe7f3f5;border-radius:999px;align-items:center;padding:2px 7px;display:inline-flex}.PA7OrW_footerItemSummary{color:var(--obc-text-secondary);word-break:break-word;font-size:11px;line-height:1.55}.PA7OrW_pinnedFooter{border-top:1px solid var(--obc-line);backdrop-filter:blur(18px);background:#ffffff80;flex:none;padding:9px 14px 12px}.PA7OrW_pinnedFooter .PA7OrW_activityFooter{box-shadow:none;backdrop-filter:none;background:0 0;border:0;border-radius:0;gap:5px;margin:0;padding:6px 2px 2px}.PA7OrW_expression{-webkit-line-clamp:unset;display:block;overflow:visible}.PA7OrW_savedToggle{min-width:38px;min-height:26px;color:var(--obc-text-secondary);cursor:pointer;background:#f0f6ffe6;border:0;border-radius:11px;justify-content:center;align-items:center;transition:background .15s,color .15s,transform .15s;display:inline-flex}.PA7OrW_savedToggle:hover:not(:disabled){color:var(--obc-text);transform:translateY(-1px)}.PA7OrW_savedToggle:disabled{opacity:.6;cursor:wait}.PA7OrW_savedToggle[data-pressed=true].PA7OrW_watchToggle{color:var(--obc-brand-strong);background:var(--obc-brand-soft)}.PA7OrW_savedToggle[data-pressed=true].PA7OrW_starToggle{color:#e8a33d;background:#e8a33d1f}.PA7OrW_savedToggle[data-pressed=true].PA7OrW_starToggle svg{fill:currentColor}.PA7OrW_feedbackStatus{color:var(--obc-brand-strong);min-height:15px;margin-top:2px;font-size:11px;line-height:1.5}.PA7OrW_feedbackStatus[data-tone=success]{color:var(--obc-success)}.PA7OrW_feedbackStatus[data-tone=error]{color:var(--obc-danger)}.PA7OrW_commentComposer{flex-direction:column;gap:6px;margin-top:2px;display:flex}.PA7OrW_commentComposer textarea{box-sizing:border-box;width:100%;color:var(--obc-text);resize:vertical;background:#ffffffe6;border:1px solid #e6edf8eb;border-radius:10px;padding:8px 9px;font-family:inherit;font-size:11.5px}.PA7OrW_commentComposer textarea:focus{border-color:var(--obc-sky-blue);outline:none}.PA7OrW_commentComposer .PA7OrW_actionButton{align-self:flex-end}.PA7OrW_activityFooter{background:linear-gradient(#fffffffa,#f2f6fdf5);border:1px solid #dfe7f3fa;border-radius:18px;flex-direction:column;gap:10px;margin:0;padding:12px 14px;display:flex;box-shadow:0 12px 24px #495d8214}.PA7OrW_footerHead{justify-content:space-between;align-items:flex-start;gap:10px;display:flex}.PA7OrW_footerCopy{flex-direction:column;flex:auto;gap:6px;min-width:0;display:flex}.PA7OrW_footerHint{color:#41506b;min-height:18px;margin:0;padding-left:20px;font-size:12px;font-weight:700;line-height:1.6;position:relative}.PA7OrW_footerHint:before{content:\"\";background:var(--obc-sky-blue);border-radius:999px;width:9px;height:9px;position:absolute;top:50%;left:0;transform:translateY(-50%);box-shadow:0 0 0 4px #5aa9ff1f}.PA7OrW_footerHeadline{min-height:16px;color:var(--obc-text-muted);-webkit-line-clamp:2;-webkit-box-orient:vertical;margin:0;padding-left:20px;font-size:10.5px;font-weight:700;line-height:1.55;display:-webkit-box;position:relative;overflow:hidden}.PA7OrW_footerHeadline:before{content:\"\";background:#fb72994d;border-radius:999px;width:9px;height:9px;position:absolute;top:50%;left:0;transform:translateY(-50%);box-shadow:0 0 0 4px #fb729914}.PA7OrW_footerToggle{min-height:32px;color:var(--obc-text-secondary);cursor:pointer;background:#ffffffdb;border:0;border-radius:999px;flex-shrink:0;padding:7px 12px;font-size:11px;font-weight:800;line-height:1;box-shadow:0 8px 18px #495d8214}.PA7OrW_footerToggle:hover{color:var(--obc-text)}.PA7OrW_footerHistory{flex-direction:column;gap:8px;max-height:190px;display:flex;overflow-y:auto}.PA7OrW_pendingItem{border-top:1px solid var(--obc-line);grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:8px;padding:10px;display:grid}.PA7OrW_pendingCopy{gap:2px;min-width:0;display:grid}.PA7OrW_pendingCopy strong{overflow-wrap:anywhere;color:var(--obc-text);font-size:12px;font-weight:800;line-height:1.45}.PA7OrW_pendingKind,.PA7OrW_pendingConfidence{color:var(--obc-text-secondary);font-size:10.5px;font-weight:700}.PA7OrW_pendingOpen{cursor:pointer;border:1px solid var(--obc-line-strong);background:var(--obc-brand-soft);min-width:52px;min-height:34px;color:var(--obc-brand-strong);border-radius:999px;padding:6px 12px;font-size:11px;font-weight:800}.PA7OrW_pendingOpen:hover:not(:disabled){background:#fb729933}.PA7OrW_pendingOpen:disabled{opacity:.6;cursor:wait}.PA7OrW_dialogueCard{box-sizing:border-box;overflow-wrap:anywhere;border:1px solid var(--obc-line-strong);background:var(--obc-surface-strong);width:100%;box-shadow:var(--obc-shadow-sm);border-radius:14px;flex-direction:column;gap:5px;padding:12px;display:flex}.PA7OrW_dialogueCard[data-card-state=confirmed]{border-color:#30b9808c}.PA7OrW_dialogueCard[data-card-state=rejected]{border-color:#ef7a8694}.PA7OrW_dialogueCard[data-card-state=deferred]{border-color:#8f9bb09e}.PA7OrW_dialogueKicker{color:var(--obc-brand-strong);letter-spacing:.06em;font-size:10.5px;font-weight:800}.PA7OrW_dialogueTitle{color:var(--obc-text);font-size:13.5px;font-weight:700;line-height:1.55}.PA7OrW_dialogueState{color:var(--obc-text-secondary);font-size:11.5px;font-weight:700}.PA7OrW_dialogueEvidence{color:var(--obc-text-secondary);font-size:11.5px}.PA7OrW_dialogueEvidence summary{cursor:pointer;width:fit-content;color:var(--obc-text-secondary);align-items:center;font-weight:700;display:flex}.PA7OrW_dialogueEvidence ul{gap:5px;margin:3px 0 0 16px;line-height:1.55;display:grid}.PA7OrW_dialogueActions{grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin-top:6px;display:grid}.PA7OrW_dialogueAction{cursor:pointer;border:1px solid var(--obc-line);background:var(--obc-surface);min-width:0;min-height:34px;color:var(--obc-text);border-radius:999px;padding:7px;font-size:12px;font-weight:800;transition:background .15s,transform .15s}.PA7OrW_dialogueAction:hover:not(:disabled){transform:translateY(-1px)}.PA7OrW_dialogueAction:disabled{cursor:default;opacity:.5}.PA7OrW_dialogueAction.PA7OrW_action_confirm{color:var(--obc-success)}.PA7OrW_dialogueAction.PA7OrW_action_reject{color:var(--obc-danger)}.PA7OrW_dialogueAction.PA7OrW_action_discuss{color:var(--obc-brand-strong)}.PA7OrW_contextBar{border:1px solid var(--obc-line-strong);background:var(--obc-surface-soft);border-radius:12px;flex-direction:column;flex:none;gap:4px;min-width:0;padding:9px 11px;display:flex}.PA7OrW_contextBarHead{justify-content:space-between;align-items:center;gap:8px;display:flex}.PA7OrW_contextLabel{letter-spacing:.04em;color:var(--obc-brand-strong);font-size:10px;font-weight:800}.PA7OrW_contextTitle{color:var(--obc-text);overflow-wrap:anywhere;font-size:12.5px;font-weight:800;line-height:1.45}.PA7OrW_contextObservation{color:var(--obc-text-secondary);-webkit-line-clamp:2;-webkit-box-orient:vertical;font-size:11px;line-height:1.55;display:-webkit-box;overflow:hidden}.PA7OrW_contextInterpretation{color:#2b6cc4;font-size:10.5px;font-weight:600;line-height:1.5}.PA7OrW_contextClear{color:var(--obc-text-secondary);cursor:pointer;background:#fffc;border:0;border-radius:999px;flex:none;padding:4px 10px;font-size:10.5px;font-weight:700}.PA7OrW_contextClear:hover{color:var(--obc-text)}.PA7OrW_replyQuote{border-left:3px solid var(--obc-brand);background:var(--obc-brand-soft);border-radius:8px;flex-direction:column;gap:1px;min-width:0;padding:6px 9px;display:flex}.PA7OrW_replyQuote span{color:var(--obc-brand-strong);letter-spacing:.04em;font-size:9.5px;font-weight:800}.PA7OrW_replyQuote strong{color:var(--obc-text-secondary);text-overflow:ellipsis;white-space:nowrap;font-size:11px;line-height:1.4;overflow:hidden}.PA7OrW_viewIntro{padding:2px 4px 0}.PA7OrW_viewKicker{letter-spacing:.08em;text-transform:uppercase;color:var(--obc-brand);font-size:9px;font-weight:700}.PA7OrW_viewIntro h2{color:var(--obc-text);letter-spacing:-.02em;margin:2px 0 0;font-size:15px;font-weight:800;line-height:1.35}.PA7OrW_viewIntro p{color:var(--obc-text-secondary);margin:3px 0 0;font-size:11px;line-height:1.6}.PA7OrW_profileCard{flex-direction:column;gap:10px;display:flex}.PA7OrW_profileSummary{background:#fcfafbf5;border:1px solid #e4dce8b3;border-radius:16px;padding:14px 16px}.PA7OrW_profilePortraitP{color:var(--obc-text);text-indent:1.6em;margin:0;font-size:12.5px;line-height:1.85}.PA7OrW_profilePortraitP:first-child{text-indent:0}.PA7OrW_profileLayer{text-transform:uppercase;letter-spacing:.05em;color:var(--obc-text-muted);border-bottom:1px solid #e7eef899;margin-top:10px;margin-bottom:2px;padding:5px 0;font-size:9.5px;font-weight:700}.PA7OrW_profileGroup{background:#fffffff0;border:1px solid #e7eef8f5;border-radius:16px;flex-direction:column;gap:8px;padding:12px 14px;display:flex}.PA7OrW_profileGroup h3{color:var(--obc-text);margin:0;font-size:12px;font-weight:800;line-height:1.4}.PA7OrW_cognitionCard{background:#fffffff0;border:1px solid #e7eef8f5;border-radius:14px;padding:8px 10px}.PA7OrW_cognitionToggle{text-align:left;cursor:pointer;width:100%;color:var(--obc-text);background:0 0;border:0;flex-direction:column;gap:3px;padding:0;display:flex}.PA7OrW_cognitionToggle:disabled{cursor:default}.PA7OrW_cognitionSummary{font-size:11.5px;font-weight:700;line-height:1.5}.PA7OrW_cognitionContext{color:var(--obc-text-secondary);font-size:10.5px;line-height:1.45}.PA7OrW_cognitionMeta{color:var(--obc-text-muted);flex-wrap:wrap;gap:6px;font-size:9.5px;display:flex}.PA7OrW_cognitionSource{color:var(--obc-brand-strong)}.PA7OrW_cognitionDetails{border-top:1px dashed #d9e3f2f2;flex-direction:column;gap:5px;margin-top:7px;padding-top:7px;display:flex}.PA7OrW_cognitionDetail{color:var(--obc-text-secondary);font-size:10.5px;line-height:1.55}.PA7OrW_cognitionDetailLabel{color:var(--obc-text);margin-right:5px;font-weight:800}.PA7OrW_profilePhaseCopy{color:var(--obc-text-secondary);margin:0;font-size:12px;line-height:1.7}.PA7OrW_chip[data-tone=brand]{background:var(--obc-brand-soft);color:var(--obc-brand-strong)}.PA7OrW_chip[data-tone=success]{color:var(--obc-success);background:#30b9801f}.PA7OrW_chip[data-tone=danger]{color:var(--obc-danger);background:#ef7a861f}.PA7OrW_chipWeight{opacity:.75;margin-left:3px;font-size:9px}.PA7OrW_mbtiContainer{flex-direction:column;gap:8px;display:flex}.PA7OrW_mbtiTypeRow{align-items:baseline;gap:8px;display:flex}.PA7OrW_mbtiTypeLabel{color:var(--obc-brand-strong);letter-spacing:.02em;font-size:18px;font-weight:800}.PA7OrW_mbtiConfidence{color:var(--obc-text-muted);background:#e7eef8f5;border-radius:999px;padding:2px 8px;font-size:9.5px;font-weight:600}.PA7OrW_mbtiDimensions{flex-direction:column;gap:6px;display:flex}.PA7OrW_mbtiDimRow{align-items:center;gap:8px;font-size:10.5px;display:flex}.PA7OrW_mbtiDimPole{text-align:center;width:20px;color:var(--obc-brand-strong);font-weight:700}.PA7OrW_mbtiDimBar{background:#e7eef8f5;border-radius:3px;flex:1;height:6px;overflow:hidden}.PA7OrW_mbtiDimBarFill{background:linear-gradient(90deg,#f7828e,#c7698f);border-radius:3px;height:100%}.PA7OrW_mbtiDimPct{text-align:right;width:32px;color:var(--obc-text-muted);font-size:9.5px}.PA7OrW_interestTree{flex-direction:column;gap:8px;display:flex}.PA7OrW_interestTreeLabel{font-size:11.5px;font-weight:700}.PA7OrW_interestTreeLabel[data-tone=sky]{color:var(--obc-sky-blue)}.PA7OrW_interestTreeLabel[data-tone=danger]{color:var(--obc-danger)}.PA7OrW_interestDomain{flex-direction:column;gap:5px;display:flex}.PA7OrW_interestDomainHeader{align-items:center;gap:6px;display:flex}.PA7OrW_interestDomainName{color:var(--obc-text);font-size:12px;font-weight:800}.PA7OrW_interestDomainWeight{color:var(--obc-text-muted);background:#e7eef8f5;border-radius:999px;padding:1px 7px;font-size:9.5px;font-weight:600}.PA7OrW_barRow{align-items:center;gap:8px;font-size:11px;display:flex}.PA7OrW_barLabel{min-width:64px;color:var(--obc-text-secondary)}.PA7OrW_barTrack{background:#e7eef8f5;border-radius:3px;flex:1;height:6px;overflow:hidden}.PA7OrW_barFill{background:linear-gradient(90deg,#397fcb,#5e9fd9);border-radius:3px;height:100%}.PA7OrW_barPct{text-align:right;width:34px;color:var(--obc-text-muted);font-size:9.5px}.PA7OrW_contextRow{gap:8px;font-size:11px;display:flex}.PA7OrW_contextLabel{color:var(--obc-text-secondary);flex:none;min-width:44px}.PA7OrW_contextValue{color:var(--obc-text)}.PA7OrW_probe[data-tone=interest]{background:linear-gradient(135deg,#fff8e8,#fff0d5);border-color:#dd7f2d47}.PA7OrW_probe[data-tone=avoidance]{background:linear-gradient(135deg,#eef7ff,#e1effc);border-color:#3b82f647}.PA7OrW_probeHead{justify-content:space-between;align-items:center;gap:8px;display:flex}.PA7OrW_insightCard{border:1px solid var(--obc-line);background:#fffffff0;border-radius:14px;flex-direction:column;gap:7px;padding:12px 14px;display:flex}.PA7OrW_insightCard[data-validated]{border-color:#30b98080}.PA7OrW_insightHead{align-items:flex-start;gap:6px;display:flex}.PA7OrW_insightTitle{color:var(--obc-text);flex:1;font-size:12px;font-weight:800;line-height:1.5}.PA7OrW_insightValidated{color:var(--obc-success);background:#30b9801f;border-radius:999px;flex:none;padding:2px 7px;font-size:9.5px;font-weight:700}.PA7OrW_insightConfidenceRow{align-items:center;gap:8px;display:flex}.PA7OrW_insightConfidenceBar{background:#e7eef8f5;border-radius:3px;flex:1;height:6px;overflow:hidden}.PA7OrW_insightConfidenceFill{background:linear-gradient(90deg,#f7828e,#c7698f);border-radius:3px;height:100%}.PA7OrW_insightConfidenceLabel{color:var(--obc-text-muted);text-align:right;width:32px;font-size:9.5px}.PA7OrW_insightEvidenceList{color:var(--obc-text-secondary);gap:4px;margin:0;padding-left:16px;font-size:11px;line-height:1.55;display:grid}.PA7OrW_awarenessList{flex-direction:column;gap:8px;display:flex}.PA7OrW_awarenessItem{background:#fffcf7f7;border:1px solid #f0e6d7e6;border-radius:14px;flex-direction:column;gap:4px;padding:10px 13px;display:flex}.PA7OrW_awarenessHeader{align-items:center;gap:8px;display:flex}.PA7OrW_awarenessItemDate{color:var(--obc-text-muted);white-space:nowrap;font-size:10px}.PA7OrW_awarenessEmotion{color:var(--obc-brand);font-size:10px;font-weight:600}.PA7OrW_awarenessObservation{color:var(--obc-text);font-size:11.5px;line-height:1.65}.PA7OrW_awarenessTrend{color:var(--obc-text-secondary);border-left:2px solid #fb729940;padding-left:8px;font-size:11px;line-height:1.5}.PA7OrW_questionCard{box-sizing:border-box;width:100%;box-shadow:var(--obc-shadow-sm);background:linear-gradient(135deg,#eef7ff,#e1effc);border:1px solid #3b82f647;border-radius:14px;flex-direction:column;gap:6px;padding:12px;display:flex}.PA7OrW_pendingObservation{color:var(--obc-text-secondary);font-size:11px;line-height:1.55}.PA7OrW_pendingInterpretation{color:var(--obc-sky-blue);font-size:10.5px;font-weight:600;line-height:1.5}.PA7OrW_questionObservation{color:var(--obc-text);background:#ffffffb3;border-radius:8px;padding:6px 9px;font-size:11.5px;line-height:1.6}.PA7OrW_questionInterpretation{color:#2b6cc4;font-size:11px;font-weight:600;line-height:1.55}.PA7OrW_delightCard{background:radial-gradient(circle at 100% 0,#fb729924,#0000 38%),linear-gradient(#fffcfefa,#f6fafffa);border:1px solid #fb729938;border-radius:16px;flex-direction:column;grid-template-columns:none;gap:0;padding:0;transition:border-color .2s,box-shadow .2s;display:flex;overflow:hidden;box-shadow:inset 0 1px #ffffffd1,0 4px 14px #fb72991a}.PA7OrW_delightKicker{color:var(--obc-brand-strong);letter-spacing:.02em;white-space:nowrap;background:#fb72991a;border-radius:999px;padding:2px 7px;font-size:9.5px;font-weight:700}.PA7OrW_delightPlatform{color:var(--obc-text-muted);white-space:nowrap;background:#0000000d;border-radius:999px;flex:none;padding:2px 6px;font-size:9.5px;font-weight:600}.PA7OrW_delightCounter{color:var(--obc-text-muted);white-space:nowrap;flex:none;font-size:10px;font-weight:600}.PA7OrW_delightNav{width:26px;height:26px;color:var(--obc-brand-strong);cursor:pointer;background:#fffc;border:1px solid #fb729938;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;font-size:14px;font-weight:700;line-height:1;transition:background .15s,border-color .15s;display:inline-flex}.PA7OrW_delightNav:hover:not(:disabled){background:#fb729924;border-color:#fb729973}.PA7OrW_delightNav:disabled{opacity:.32;cursor:default}.PA7OrW_delightDismiss{width:26px;height:26px;color:var(--obc-text-secondary);cursor:pointer;background:#0000000a;border:0;border-left-width:medium;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;font-size:15px;line-height:1;transition:background .15s,color .15s;display:inline-flex}.PA7OrW_delightRow{cursor:pointer;text-align:left;min-width:0;color:inherit;background:0 0;border:0;align-items:center;gap:10px;padding:8px 10px 10px;display:flex}.PA7OrW_delightThumb{aspect-ratio:auto;object-fit:cover;background:linear-gradient(145deg,#fb72992e,#a855f71f);border:1px solid #ffffffeb;border-radius:10px;flex:none;width:104px;height:58px;box-shadow:inset 0 1px #ffffffb8,0 2px 8px #fb72991f}.PA7OrW_delightThumbFallback{aspect-ratio:auto;box-sizing:border-box;width:104px;height:58px;color:var(--obc-text-secondary);text-align:center;background:linear-gradient(145deg,#fb72992e,#a855f71f);border:1px solid #ffffffeb;border-radius:10px;flex:none;justify-content:center;align-items:center;padding:4px;font-size:9px;font-weight:700;line-height:1.4;display:flex;overflow:hidden;box-shadow:inset 0 1px #ffffffb8}.PA7OrW_delightText{flex:1;align-items:center;min-width:0;display:flex}.PA7OrW_delightTitle{min-width:0;color:var(--obc-text);-webkit-line-clamp:2;overflow-wrap:anywhere;-webkit-box-orient:vertical;font-size:12.5px;font-weight:700;line-height:1.55;display:-webkit-box;overflow:hidden}.PA7OrW_delightChevron{width:24px;height:24px;color:var(--obc-text-muted);cursor:pointer;background:#ffffffb3;border:0;border-radius:50%;flex:none;justify-content:center;align-items:center;font-size:10px;line-height:1;display:inline-flex}.PA7OrW_delightCard{background:radial-gradient(circle at 100% 0,#fb729924,#0000 38%),linear-gradient(#fffcfefa,#f6fafffa);border:1px solid #fb729947;border-radius:18px;flex-direction:column;flex-shrink:0;gap:0;padding:0;transition:border-color .2s,box-shadow .2s;display:flex;overflow:hidden;box-shadow:inset 0 1px #ffffffd1,0 4px 14px #fb72991a}.PA7OrW_delightCard:hover{border-color:#fb729980;box-shadow:inset 0 1px #ffffffd1,0 12px 28px #fb72992e}.PA7OrW_delightHeader{align-items:center;gap:7px;min-width:0;padding:10px 12px 2px;display:flex}.PA7OrW_delightMain{cursor:pointer;text-align:left;min-width:0;color:inherit;background:0 0;border:0;flex-direction:column;gap:8px;padding:4px 10px 10px;display:flex}.PA7OrW_delightCover{aspect-ratio:16/9;background:linear-gradient(#fffffffa,#fff8fcf0),linear-gradient(145deg,#fb72992e,#a855f71f);border:1px solid #ffffffeb;border-radius:14px;width:100%;transition:box-shadow .2s;display:block;position:relative;overflow:hidden;box-shadow:inset 0 1px #ffffffb8,0 2px 8px #fb72991f}.PA7OrW_delightCard:hover .PA7OrW_delightCover{box-shadow:inset 0 1px #ffffffb8,0 8px 18px #fb729938}.PA7OrW_delightHero{object-fit:cover;width:100%;height:100%;transition:transform .35s;display:block}.PA7OrW_delightCard:hover .PA7OrW_delightHero{transform:scale(1.03)}.PA7OrW_delightCover:before{content:\"✨\";width:27px;height:27px;color:var(--obc-brand-strong);text-align:center;pointer-events:none;z-index:2;background:#fffffff0;border-radius:999px;font-size:15px;line-height:27px;position:absolute;top:8px;left:8px;box-shadow:0 2px 8px #fb72994d}.PA7OrW_delightCoverScrim{pointer-events:none;z-index:1;background:linear-gradient(#0000,#141a2870);height:40%;position:absolute;inset:auto 0 0}.PA7OrW_delightScorePill{z-index:2;color:#fff;letter-spacing:.01em;background:linear-gradient(135deg,#fb7299f5,#e25885e6);border-radius:999px;padding:3px 10px;font-size:10.5px;font-weight:800;line-height:1.4;position:absolute;bottom:8px;left:8px;box-shadow:0 2px 8px #fb72995c}.PA7OrW_delightHeroFallback{box-sizing:border-box;width:100%;height:100%;min-height:96px;color:var(--obc-text-secondary);text-align:center;background:linear-gradient(145deg,#fb729924,#5aa9ff1a);justify-content:center;align-items:center;padding:12px 14px;font-size:11px;font-weight:700;line-height:1.55;display:flex;overflow:hidden}.PA7OrW_delightCover:has(.PA7OrW_delightHeroFallback){aspect-ratio:auto}.PA7OrW_delightTitleWrap{align-items:center;gap:8px;min-width:0;padding:0 2px;display:flex}.PA7OrW_delightTitle{min-width:0;color:var(--obc-text);-webkit-line-clamp:2;overflow-wrap:anywhere;-webkit-box-orient:vertical;flex:1;font-size:13px;font-weight:800;line-height:1.5;display:-webkit-box;overflow:hidden}.PA7OrW_delightCard[data-expanded=true] .PA7OrW_delightTitle{-webkit-line-clamp:unset;display:block}.PA7OrW_delightChevron{width:24px;height:24px;color:var(--obc-text-muted);background:#ffffffb3;border:0;border-radius:8px;flex:none;justify-content:center;align-items:center;font-size:10px;line-height:1;display:inline-flex}.PA7OrW_delightDismiss{color:var(--obc-text-muted);cursor:pointer;background:0 0;border:0;border-left:1px solid #eddbe999;border-radius:0 8px 8px 0;padding:3px 7px;font-size:15px;line-height:1;transition:background .15s,color .15s}.PA7OrW_delightDismiss:hover:not(:disabled){color:var(--obc-danger);background:#ef7a861f}.PA7OrW_delightDismiss:disabled{cursor:wait;opacity:.58}.PA7OrW_delightBody{border-top:1px solid #eddbe999;flex-direction:column;gap:8px;padding:2px 12px 12px;display:flex}.PA7OrW_delightBody .PA7OrW_delightReason{margin-top:8px}.PA7OrW_delightReason{color:var(--obc-text-secondary);background:#f0f6ffe6;border-radius:10px;padding:8px 10px;font-size:11.5px;line-height:1.6}.PA7OrW_delightScore{color:var(--obc-brand-strong);font-weight:800}.PA7OrW_delightActions{flex-wrap:wrap;gap:6px;display:flex}.PA7OrW_delightActions .PA7OrW_actionButton{flex:1;min-width:0;padding:6px 8px;font-size:11.5px}.PA7OrW_delightComposer{align-items:flex-end;gap:6px;display:flex}.PA7OrW_delightComposer .PA7OrW_chatInput{flex:1}.PA7OrW_settingsOverlay{z-index:12;backdrop-filter:blur(2px);background:#20304a47;flex-direction:column;justify-content:flex-start;display:flex;position:absolute;inset:0}.PA7OrW_settingsPanel{background:radial-gradient(circle at 15% 4%, #fb72991f, transparent 30%), var(--obc-bg);border-radius:18px 18px 0 0;flex-direction:column;flex:1;min-height:0;margin-top:8px;display:flex}.PA7OrW_settingsHeader{border-bottom:1px solid var(--obc-line);flex:none;justify-content:space-between;align-items:center;padding:14px 14px 10px;display:flex}.PA7OrW_settingsHeader h2{color:var(--obc-text);letter-spacing:-.02em;margin:0;font-size:15px;font-weight:800}.PA7OrW_settingsBack{width:30px;height:30px;color:var(--obc-text-secondary);cursor:pointer;background:#ffffffc2;border:1px solid #e6edf8eb;border-radius:10px;font-size:14px}.PA7OrW_settingsBack:hover{background:var(--obc-brand-soft);color:var(--obc-brand-strong)}.PA7OrW_settingsTabs{border-bottom:1px solid var(--obc-line);scrollbar-width:none;flex:none;gap:2px;padding:8px 10px 0;display:flex;overflow-x:auto}.PA7OrW_settingsTab{color:var(--obc-text-secondary);cursor:pointer;white-space:nowrap;background:0 0;border:0;border-radius:8px 8px 0 0;padding:6px 10px 8px;font-size:11.5px;font-weight:700}.PA7OrW_settingsTab:hover{color:var(--obc-text);background:var(--obc-interactive-bg-hover,#ffffff80)}.PA7OrW_settingsTab[data-active=true]{color:var(--obc-brand-strong);box-shadow:inset 0 -2px 0 var(--obc-brand);font-weight:800}.PA7OrW_settingsBody{flex-direction:column;flex:1;gap:10px;min-height:0;padding:12px 14px 24px;display:flex;overflow-y:auto}.PA7OrW_settingsToastBar{color:var(--obc-success);background:#30b9801f;border:1px solid #30b9804d;border-radius:8px;flex:none;padding:7px 10px;font-size:11.5px}.PA7OrW_settingsSection{background:#fffffff0;border:1px solid #e7eef8f5;border-radius:16px;flex-direction:column;gap:10px;padding:12px 14px;display:flex}.PA7OrW_settingsSection h3{color:var(--obc-text);align-items:center;gap:6px;margin:0;font-size:13px;font-weight:800;display:flex}.PA7OrW_sectionIcon{font-size:14px}.PA7OrW_settingsField{flex-direction:column;gap:4px;display:flex}.PA7OrW_settingsField>label{color:var(--obc-text-secondary);font-size:11px;font-weight:700}.PA7OrW_settingsFieldRow{color:var(--obc-text);align-items:center;gap:8px;font-size:11.5px;display:flex}.PA7OrW_settingsFieldRow input[type=checkbox]{accent-color:var(--obc-brand)}.PA7OrW_settingsInput{box-sizing:border-box;width:100%;color:var(--obc-text);background:#ffffffe6;border:1px solid #e6edf8eb;border-radius:8px;padding:6px 9px;font-family:inherit;font-size:11.5px}.PA7OrW_settingsInput:focus{border-color:var(--obc-sky-blue);outline:none}.PA7OrW_settingsHint{color:var(--obc-text-muted);margin:0;font-size:10.5px;line-height:1.5}.PA7OrW_settingsActions{align-items:center;gap:8px;display:flex}.PA7OrW_settingsToast{color:var(--obc-success);font-size:11px}.PA7OrW_settingsInstance{border:1px solid var(--obc-line);background:#ffffffb3;border-radius:12px;flex-direction:column;gap:8px;padding:10px 11px;display:flex}.PA7OrW_settingsInstanceHead{color:var(--obc-text);justify-content:space-between;align-items:center;gap:8px;font-size:12px;font-weight:800;display:flex}.PA7OrW_settingsSavebar{border:1px solid var(--obc-line);backdrop-filter:blur(14px);box-shadow:var(--obc-shadow-sm);background:#ffffffd1;border-radius:14px;justify-content:flex-end;align-items:center;gap:10px;margin-top:12px;padding:10px 12px;display:flex;position:sticky;bottom:0}.PA7OrW_settingsSavebarMsg{color:var(--obc-text-muted);flex:1;font-size:11px}.PA7OrW_probeStatus{color:var(--obc-text-muted);word-break:break-word;font-size:11px;line-height:1.5}.PA7OrW_probeStatus[data-tone=success]{color:var(--obc-success)}.PA7OrW_probeStatus[data-tone=error]{color:var(--obc-danger)}.PA7OrW_llmInstanceList{flex-direction:column;gap:8px;display:flex}.PA7OrW_llmInstance{border:1px solid var(--obc-line);background:#ffffffb3;border-radius:12px;justify-content:space-between;align-items:center;gap:10px;padding:9px 11px;display:flex}.PA7OrW_llmInstanceMain{flex-direction:column;gap:3px;min-width:0;display:flex}.PA7OrW_llmInstanceHead{flex-wrap:wrap;align-items:center;gap:6px;display:flex}.PA7OrW_llmInstanceName{color:var(--obc-text);font-size:12px;font-weight:800}.PA7OrW_llmBadge{background:var(--obc-brand-soft);color:var(--obc-brand-strong);border-radius:999px;padding:3px 7px;font-size:9px;font-weight:800;line-height:1}.PA7OrW_llmBadge[data-tone=off]{color:var(--obc-text-muted);background:#8290a829}.PA7OrW_llmInstanceDetail{color:var(--obc-text-muted);text-overflow:ellipsis;white-space:nowrap;font-size:10.5px;overflow:hidden}.PA7OrW_llmInstanceActions{flex:none;gap:6px;display:flex}.PA7OrW_chainSection{border-top:1px dashed var(--obc-line);flex-direction:column;gap:8px;margin-top:10px;padding-top:10px;display:flex}.PA7OrW_chainHead{justify-content:space-between;align-items:center;gap:8px;display:flex}.PA7OrW_chainHead h4{color:var(--obc-text);margin:0;font-size:11.5px;font-weight:800}.PA7OrW_chainEditor{flex-direction:column;gap:6px;display:flex}.PA7OrW_chainList{flex-direction:column;gap:4px;display:flex}.PA7OrW_chainRow{border:1px solid var(--obc-line);background:#ffffff9e;border-radius:10px;align-items:center;gap:4px;padding:5px 8px;display:flex}.PA7OrW_chainName{min-width:0;color:var(--obc-text);text-overflow:ellipsis;white-space:nowrap;flex:1;font-size:11px;font-weight:700;overflow:hidden}.PA7OrW_chainBtn{border:1px solid var(--obc-line);width:22px;height:22px;color:var(--obc-text-muted);cursor:pointer;background:#ffffffb3;border-radius:7px;flex:none;justify-content:center;align-items:center;font-size:10px;line-height:1;display:inline-flex}.PA7OrW_chainBtn:hover:not(:disabled){color:var(--obc-brand-strong);border-color:var(--obc-brand)}.PA7OrW_chainBtn:disabled{opacity:.35;cursor:default}.PA7OrW_chainPicker{align-items:center;gap:6px;display:flex}.PA7OrW_chainPicker .PA7OrW_settingsInput{flex:1;min-width:0}.PA7OrW_moduleRoute{border:1px solid var(--obc-line);background:#ffffff9e;border-radius:12px;margin-bottom:8px;padding:8px 10px}.PA7OrW_dialogOverlay{backdrop-filter:blur(3px);z-index:60;background:#141a286b;justify-content:center;align-items:flex-start;padding:6vh 14px;display:flex;position:fixed;inset:0}.PA7OrW_dialogCard{border:1px solid var(--obc-line);background:var(--obc-bg);width:min(460px,100%);max-height:86vh;box-shadow:var(--obc-shadow-lg);border-radius:16px;flex-direction:column;gap:10px;padding:14px;display:flex;overflow-y:auto}.PA7OrW_dialogHead{justify-content:space-between;align-items:center;gap:8px;display:flex}.PA7OrW_dialogHead h3{color:var(--obc-text);margin:0;font-size:13px;font-weight:800}.PA7OrW_dialogFields{flex-direction:column;gap:8px;display:flex}.PA7OrW_dialogActionRow{align-items:center;gap:6px;display:flex}.PA7OrW_dialogActionRow .PA7OrW_settingsInput{flex:1;min-width:0}.PA7OrW_dialogActions{border-top:1px solid var(--obc-line);flex-wrap:wrap;align-items:center;gap:8px;padding-top:10px;display:flex}.PA7OrW_dialogActions .PA7OrW_probeStatus{flex:100%;order:-1}.PA7OrW_updateRow{color:var(--obc-text-secondary);justify-content:space-between;align-items:baseline;gap:10px;padding:4px 0;font-size:11.5px;display:flex}.PA7OrW_updateRow strong{color:var(--obc-text);word-break:break-word;text-align:right;font-weight:800}.PA7OrW_panel[data-dark=true]{--obc-bg:#15161a;--obc-sky:#0f1115;--obc-surface:#24262ce0;--obc-surface-strong:#282a31f7;--obc-surface-soft:#212329d1;--obc-line:#ffffff14;--obc-line-strong:#fb729959;--obc-text:#e9ebf1;--obc-text-secondary:#b0b9ca;--obc-text-muted:#8c97ad;--obc-brand-soft:#fb729929;--obc-sky-soft:#5aa9ff29;--obc-shadow-lg:0 20px 40px #00000073;--obc-shadow-sm:0 10px 22px #00000059;background:radial-gradient(circle at 15% 8%,#fb72991a,#0000 30%),radial-gradient(circle at 88% 12%,#5aa9ff1a,#0000 26%),linear-gradient(#17181d 0%,#101216 100%)}.PA7OrW_panel[data-dark=true] .PA7OrW_iconButton,.PA7OrW_panel[data-dark=true] .PA7OrW_settingsBack{color:var(--obc-text-secondary);background:#ffffff0f;border-color:#ffffff1a}.PA7OrW_panel[data-dark=true] .PA7OrW_iconButton:hover,.PA7OrW_panel[data-dark=true] .PA7OrW_settingsBack:hover{background:var(--obc-brand-soft);color:var(--obc-brand-strong)}.PA7OrW_panel[data-dark=true] .PA7OrW_tab:hover{background:#ffffff0f}.PA7OrW_panel[data-dark=true] .PA7OrW_tab[data-active=true]{background:linear-gradient(#ffffff1a,#fb729924);box-shadow:0 10px 18px #0000004d}.PA7OrW_panel[data-dark=true] .PA7OrW_recHeader,.PA7OrW_panel[data-dark=true] .PA7OrW_card,.PA7OrW_panel[data-dark=true] .PA7OrW_recCard,.PA7OrW_panel[data-dark=true] .PA7OrW_profileGroup,.PA7OrW_panel[data-dark=true] .PA7OrW_settingsSection,.PA7OrW_panel[data-dark=true] .PA7OrW_dialogueCard,.PA7OrW_panel[data-dark=true] .PA7OrW_insightCard,.PA7OrW_panel[data-dark=true] .PA7OrW_probe,.PA7OrW_panel[data-dark=true] .PA7OrW_footerItem{box-shadow:none;background:linear-gradient(#26282feb,#21232aeb);border-color:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_card:hover,.PA7OrW_panel[data-dark=true] .PA7OrW_recCard:hover{border-color:#fb729973;box-shadow:0 14px 26px #0006}.PA7OrW_panel[data-dark=true] .PA7OrW_delightCard{background:radial-gradient(circle at 100% 0,#fb72991f,#0000 38%),linear-gradient(#2c242cf7,#1e222cf7);border-color:#fb72994d;box-shadow:inset 0 1px #ffffff0d,0 4px 14px #0000004d}.PA7OrW_panel[data-dark=true] .PA7OrW_delightCard:hover{border-color:#fb72998c;box-shadow:inset 0 1px #ffffff0d,0 12px 28px #0006}.PA7OrW_panel[data-dark=true] .PA7OrW_delightKicker,.PA7OrW_panel[data-dark=true] .PA7OrW_topicBadge,.PA7OrW_panel[data-dark=true] .PA7OrW_chip[data-tone=brand]{background:#fb729929}.PA7OrW_panel[data-dark=true] .PA7OrW_delightPlatform,.PA7OrW_panel[data-dark=true] .PA7OrW_stateBadge,.PA7OrW_panel[data-dark=true] .PA7OrW_platformTag,.PA7OrW_panel[data-dark=true] .PA7OrW_poolChip,.PA7OrW_panel[data-dark=true] .PA7OrW_chip,.PA7OrW_panel[data-dark=true] .PA7OrW_contextBadge{color:var(--obc-text-secondary);background:#ffffff12}.PA7OrW_panel[data-dark=true] .PA7OrW_delightNav,.PA7OrW_panel[data-dark=true] .PA7OrW_delightChevron,.PA7OrW_panel[data-dark=true] .PA7OrW_footerToggle,.PA7OrW_panel[data-dark=true] .PA7OrW_pendingOpen,.PA7OrW_panel[data-dark=true] .PA7OrW_contextClear{background:#ffffff12}.PA7OrW_panel[data-dark=true] .PA7OrW_actionButton{color:var(--obc-text-secondary);background:#ffffff12}.PA7OrW_panel[data-dark=true] .PA7OrW_actionButton:hover:not(:disabled){color:var(--obc-text);background:#ffffff1f}.PA7OrW_panel[data-dark=true] .PA7OrW_savedToggle{color:var(--obc-text-secondary);background:#ffffff12}.PA7OrW_panel[data-dark=true] .PA7OrW_settingsInput,.PA7OrW_panel[data-dark=true] .PA7OrW_commentInput,.PA7OrW_panel[data-dark=true] .PA7OrW_chatInput,.PA7OrW_panel[data-dark=true] .PA7OrW_commentComposer textarea{color:var(--obc-text);background:#181a1fe6;border-color:#ffffff1a}.PA7OrW_panel[data-dark=true] .PA7OrW_turnSoul,.PA7OrW_panel[data-dark=true] .PA7OrW_questionObservation{background:#ffffff0f;border-color:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_turnUser{background:#fb729933}.PA7OrW_panel[data-dark=true] .PA7OrW_profileSummary{background:#242028f0;border-color:#ffffff12}.PA7OrW_panel[data-dark=true] .PA7OrW_awarenessItem{background:#28241ef0;border-color:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_questionCard{background:linear-gradient(135deg,#1a2433,#141d2c);border-color:#3b82f659}.PA7OrW_panel[data-dark=true] .PA7OrW_messageCard[data-tone=interest]{background:linear-gradient(135deg,#2b2418,#241d12);border-color:#dd7f2d66}.PA7OrW_panel[data-dark=true] .PA7OrW_messageCard[data-tone=avoidance]{background:linear-gradient(135deg,#141f2e,#101a26);border-color:#3b82f666}.PA7OrW_panel[data-dark=true] .PA7OrW_messageCard[data-tone=challenge]{background:linear-gradient(135deg,#221d33,#1b1728);border-color:#7c3aed66}.PA7OrW_panel[data-dark=true] .PA7OrW_messageCard[data-tone=delight]{background:linear-gradient(135deg,#2e1f26,#261a20);border-color:#fb729966}.PA7OrW_panel[data-dark=true] .PA7OrW_probe[data-tone=interest]{background:linear-gradient(135deg,#2b2418,#241d12);border-color:#dd7f2d66}.PA7OrW_panel[data-dark=true] .PA7OrW_probe[data-tone=avoidance]{background:linear-gradient(135deg,#141f2e,#101a26);border-color:#3b82f666}.PA7OrW_panel[data-dark=true] .PA7OrW_messageBody{color:#b4bece}.PA7OrW_panel[data-dark=true] .PA7OrW_mbtiDimBar,.PA7OrW_panel[data-dark=true] .PA7OrW_barTrack,.PA7OrW_panel[data-dark=true] .PA7OrW_insightConfidenceBar,.PA7OrW_panel[data-dark=true] .PA7OrW_mbtiConfidence{background:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_drawerPanel,.PA7OrW_panel[data-dark=true] .PA7OrW_settingsPanel{background:radial-gradient(circle at 15% 4%,#fb729914,#0000 30%),#15161a}.PA7OrW_panel[data-dark=true] .PA7OrW_settingsInstance{background:#ffffff0a;border-color:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_pendingItem{border-top-color:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_contextBar,.PA7OrW_panel[data-dark=true] .PA7OrW_activityFooter{background:#212329d9;border-color:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_subTabs{background:#ffffff0a;border-color:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_subTab[data-active=true]{background:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_delightReason{color:var(--obc-text-secondary);background:#ffffff0f}.PA7OrW_panel[data-dark=true] .PA7OrW_delightBody{border-top-color:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_delightChevron{color:var(--obc-text-secondary);background:#ffffff12}.PA7OrW_panel[data-dark=true] .PA7OrW_delightDismiss{border-left-color:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_footerHint{color:#c3cddd}.PA7OrW_panel[data-dark=true] .PA7OrW_settingsSavebar,.PA7OrW_panel[data-dark=true] .PA7OrW_llmInstance,.PA7OrW_panel[data-dark=true] .PA7OrW_chainRow,.PA7OrW_panel[data-dark=true] .PA7OrW_moduleRoute{background:#24262ce0;border-color:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_chainBtn{background:#ffffff0f;border-color:#ffffff1a}.PA7OrW_panel[data-dark=true] .PA7OrW_dialogCard{background:radial-gradient(circle at 15% 4%,#fb729914,#0000 30%),#15161a;border-color:#ffffff1a}.PA7OrW_panel[data-dark=true] .PA7OrW_llmBadge{background:#fb729933}.PA7OrW_panel[data-dark=true] .PA7OrW_actionButton:disabled{opacity:.66;color:#9aa4b8}.PA7OrW_panel[data-dark=true] .PA7OrW_recCover{background:linear-gradient(145deg,#fb729924,#5aa9ff1f),#1e2028f5}.PA7OrW_panel[data-dark=true] .PA7OrW_delightCover{background:linear-gradient(#282a31f5,#212329f5),linear-gradient(145deg,#fb729929,#5aa9ff1a);border-color:#ffffff24;box-shadow:inset 0 1px #ffffff0f,0 2px 8px #0000004d}.PA7OrW_panel[data-dark=true] .PA7OrW_delightCard:hover .PA7OrW_delightCover{box-shadow:inset 0 1px #ffffff0f,0 8px 18px #0000006b}.PA7OrW_panel[data-dark=true] .PA7OrW_delightHeroFallback{color:var(--obc-text);background:linear-gradient(145deg,#fb729929,#5aa9ff1f)}.PA7OrW_panel[data-dark=true] .PA7OrW_pinnedFooter{background:#15161ab3;border-top-color:#ffffff14}.PA7OrW_panel[data-dark=true] .PA7OrW_pinnedFooter .PA7OrW_activityFooter{box-shadow:none;background:0 0;border:0}.PA7OrW_panel[data-dark=true] .PA7OrW_starButton{background:linear-gradient(#2c242cf7,#fb72991f);border-color:#fb729952}.PA7OrW_sidebarButton{box-sizing:border-box;cursor:pointer;width:calc(100% + 8px);height:34px;color:var(--dsw-alias-label-primary,#20304a);background:0 0;border:none;border-radius:12px;flex:none;align-items:center;gap:8px;margin:4px -4px;padding:6px 2px 6px 10px;font-family:inherit;font-size:13px;font-weight:600;line-height:22px;display:flex;overflow:hidden}.PA7OrW_sidebarButton:hover{background:var(--dsw-alias-interactive-bg-hover,#0000000d)}.PA7OrW_sidebarButton[data-active=true]{color:#f65788;background:#fb72991f}.PA7OrW_sidebarButton[data-wide=false]{border-radius:50%;justify-content:center;gap:0;width:36px;height:36px;margin:8px 0 10px;padding:0}.PA7OrW_sidebarButtonIcon{object-fit:cover;background:linear-gradient(135deg,#fb7299,#ff8cb0);border-radius:6px;flex:none;width:18px;height:18px;display:block;box-shadow:0 4px 8px #fb729940}.PA7OrW_sidebarButtonLabel{white-space:nowrap;overflow:hidden}.obc-panel-handle:after{content:\"\";background:#20304a2e;border-radius:2px;width:2px;height:32px;transition:background .15s;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%)}.obc-panel-handle:hover:after{background:#fb72998c}";
		...
		var panel_module_css_default = {
			"settingsToast": "PA7OrW_settingsToast",
			"profileSummary": "PA7OrW_profileSummary",
			"settingsSection": "PA7OrW_settingsSection",
			"settings": "PA7OrW_settings",
			"dialogueState": "PA7OrW_dialogueState",
			"dialogueActions": "PA7OrW_dialogueActions",
			"cardBody": "PA7OrW_cardBody",
			"poolChips": "PA7OrW_poolChips",
			"cardTitle": "PA7OrW_cardTitle",
			"delightThumb": "PA7OrW_delightThumb",
			"mbtiDimRow": "PA7OrW_mbtiDimRow",
			"settingsOverlay": "PA7OrW_settingsOverlay",
			"llmBadge": "PA7OrW_llmBadge",
			"questionObservation": "PA7OrW_questionObservation",
			"domainWeight": "PA7OrW_domainWeight",
			"syncLineDetail": "PA7OrW_syncLineDetail",
			"drawerHeader": "PA7OrW_drawerHeader",
			"mbtiDimensions": "PA7OrW_mbtiDimensions",
			"delightComposer": "PA7OrW_delightComposer",
			"contextTitle": "PA7OrW_contextTitle",
			"starButton": "PA7OrW_starButton",
			"starCount": "PA7OrW_starCount",
			"contextBar": "PA7OrW_contextBar",
			"hint": "PA7OrW_hint",
			"chatInputRow": "PA7OrW_chatInputRow",
			"replyQuote": "PA7OrW_replyQuote",
			"dialogueKicker": "PA7OrW_dialogueKicker",
			"drawerOverlay": "PA7OrW_drawerOverlay",
			"settingsToastBar": "PA7OrW_settingsToastBar",
			"probeStatus": "PA7OrW_probeStatus",
			"barLabel": "PA7OrW_barLabel",
			"pendingConfidence": "PA7OrW_pendingConfidence",
			"card": "PA7OrW_card",
			"dialogActionRow": "PA7OrW_dialogActionRow",
			"cognitionDetails": "PA7OrW_cognitionDetails",
			"delightTitleWrap": "PA7OrW_delightTitleWrap",
			"expression": "PA7OrW_expression",
			"dialogHead": "PA7OrW_dialogHead",
			"awarenessHeader": "PA7OrW_awarenessHeader",
			"mbtiDimBar": "PA7OrW_mbtiDimBar",
			"questionInterpretation": "PA7OrW_questionInterpretation",
			"barRow": "PA7OrW_barRow",
			"pendingOpen": "PA7OrW_pendingOpen",
			"probeReason": "PA7OrW_probeReason",
			"delightActions": "PA7OrW_delightActions",
			"tabBar": "PA7OrW_tabBar",
			"awarenessList": "PA7OrW_awarenessList",
			"delightBody": "PA7OrW_delightBody",
			"probeActions": "PA7OrW_probeActions",
			"chainEditor": "PA7OrW_chainEditor",
			"settingsSavebar": "PA7OrW_settingsSavebar",
			"insightHead": "PA7OrW_insightHead",
			"settingsHeader": "PA7OrW_settingsHeader",
			"barTrack": "PA7OrW_barTrack",
			"insightEvidenceList": "PA7OrW_insightEvidenceList",
			"coverWrap": "PA7OrW_coverWrap",
			"insightConfidenceFill": "PA7OrW_insightConfidenceFill",
			"messageType": "PA7OrW_messageType",
			"insightTitle": "PA7OrW_insightTitle",
			"sectionTitle": "PA7OrW_sectionTitle",
			"header": "PA7OrW_header",
			"recBody": "PA7OrW_recBody",
			"settingsHint": "PA7OrW_settingsHint",
			"llmInstanceActions": "PA7OrW_llmInstanceActions",
			"delightThumbFallback": "PA7OrW_delightThumbFallback",
			"llmInstance": "PA7OrW_llmInstance",
			"delightKicker": "PA7OrW_delightKicker",
			"turn": "PA7OrW_turn",
			"insightConfidenceRow": "PA7OrW_insightConfidenceRow",
			"recHeaderActions": "PA7OrW_recHeaderActions",
			"coverCorner": "PA7OrW_coverCorner",
			"llmInstanceDetail": "PA7OrW_llmInstanceDetail",
			"activityTime": "PA7OrW_activityTime",
			"messagePrompt": "PA7OrW_messagePrompt",
			"recHeaderCopy": "PA7OrW_recHeaderCopy",
			"footerToggle": "PA7OrW_footerToggle",
			"settingsFieldRow": "PA7OrW_settingsFieldRow",
			"insightConfidenceLabel": "PA7OrW_insightConfidenceLabel",
			"delightChevron": "PA7OrW_delightChevron",
			"settingsInput": "PA7OrW_settingsInput",
			"chainName": "PA7OrW_chainName",
			"awarenessRow": "PA7OrW_awarenessRow",
			"sectionIcon": "PA7OrW_sectionIcon",
			"dialogueCard": "PA7OrW_dialogueCard",
			"mbtiDimPole": "PA7OrW_mbtiDimPole",
			"moduleRoute": "PA7OrW_moduleRoute",
			"footerHead": "PA7OrW_footerHead",
			"chip": "PA7OrW_chip",
			"cardActions": "PA7OrW_cardActions",
			"poolChipValue": "PA7OrW_poolChipValue",
			"thumb": "PA7OrW_thumb",
			"contextClear": "PA7OrW_contextClear",
			"mbtiDimPct": "PA7OrW_mbtiDimPct",
			"dialogueTitle": "PA7OrW_dialogueTitle",
			"portrait": "PA7OrW_portrait",
			"syncLineText": "PA7OrW_syncLineText",
			"messageBody": "PA7OrW_messageBody",
			"awarenessTrend": "PA7OrW_awarenessTrend",
			"initPromptStatus": "PA7OrW_initPromptStatus",
			"delightScorePill": "PA7OrW_delightScorePill",
			"sidebarButtonIcon": "PA7OrW_sidebarButtonIcon",
			"contextBadge": "PA7OrW_contextBadge",
			"chatSend": "PA7OrW_chatSend",
			"settingsField": "PA7OrW_settingsField",
			"profileLayer": "PA7OrW_profileLayer",
			"confirmToggle": "PA7OrW_confirmToggle",
			"cardTurn": "PA7OrW_cardTurn",
			"chainRow": "PA7OrW_chainRow",
			"subTabs": "PA7OrW_subTabs",
			"drawerEmptySubtitle": "PA7OrW_drawerEmptySubtitle",
			"drawerTitle": "PA7OrW_drawerTitle",
			"contextRow": "PA7OrW_contextRow",
			"spacer": "PA7OrW_spacer",
			"awarenessEmotion": "PA7OrW_awarenessEmotion",
			"badge": "PA7OrW_badge",
			"contextLabel": "PA7OrW_contextLabel",
			"obcSpin": "PA7OrW_obcSpin",
			"delightMain": "PA7OrW_delightMain",
			"empty": "PA7OrW_empty",
			"syncChip": "PA7OrW_syncChip",
			"pendingKind": "PA7OrW_pendingKind",
			"syncToolbar": "PA7OrW_syncToolbar",
			"delightCoverScrim": "PA7OrW_delightCoverScrim",
			"chainList": "PA7OrW_chainList",
			"insightCard": "PA7OrW_insightCard",
			"settingsBack": "PA7OrW_settingsBack",
			"historyRestoreRow": "PA7OrW_historyRestoreRow",
			"contextObservation": "PA7OrW_contextObservation",
			"platformTag": "PA7OrW_platformTag",
			"settingsTabs": "PA7OrW_settingsTabs",
			"brandCopy": "PA7OrW_brandCopy",
			"status": "PA7OrW_status",
			"delightHeroFallback": "PA7OrW_delightHeroFallback",
			"mbtiTypeRow": "PA7OrW_mbtiTypeRow",
			"settingsBody": "PA7OrW_settingsBody",
			"cognitionSource": "PA7OrW_cognitionSource",
			"thumbFallback": "PA7OrW_thumbFallback",
			"activityRow": "PA7OrW_activityRow",
			"probeConfidence": "PA7OrW_probeConfidence",
			"settingsTab": "PA7OrW_settingsTab",
			"cognitionDetailLabel": "PA7OrW_cognitionDetailLabel",
			"error": "PA7OrW_error",
			"feedbackStatus": "PA7OrW_feedbackStatus",
			"syncLine": "PA7OrW_syncLine",
			"recCard": "PA7OrW_recCard",
			"dialogActions": "PA7OrW_dialogActions",
			"sidebarButtonLabel": "PA7OrW_sidebarButtonLabel",
			"syncBadgeRow": "PA7OrW_syncBadgeRow",
			"syncMessage": "PA7OrW_syncMessage",
			"pendingCopy": "PA7OrW_pendingCopy",
			"delightCounter": "PA7OrW_delightCounter",
			"interestDomainHead": "PA7OrW_interestDomainHead",
			"starToggle": "PA7OrW_starToggle",
			"action_reject": "PA7OrW_action_reject",
			"awarenessItemDate": "PA7OrW_awarenessItemDate",
			"delightHero": "PA7OrW_delightHero",
			"questionCard": "PA7OrW_questionCard",
			"viewIntro": "PA7OrW_viewIntro",
			"recCover": "PA7OrW_recCover",
			"insightValidated": "PA7OrW_insightValidated",
			"starLabel": "PA7OrW_starLabel",
			"recKicker": "PA7OrW_recKicker",
			"probeHead": "PA7OrW_probeHead",
			"settingsSavebarMsg": "PA7OrW_settingsSavebarMsg",
			"chainPicker": "PA7OrW_chainPicker",
			"cognitionDetail": "PA7OrW_cognitionDetail",
			"delightReason": "PA7OrW_delightReason",
			"brandMark": "PA7OrW_brandMark",
			"chipRow": "PA7OrW_chipRow",
			"action_discuss": "PA7OrW_action_discuss",
			"barPct": "PA7OrW_barPct",
			"historyRestoreLabel": "PA7OrW_historyRestoreLabel",
			"messageTitle": "PA7OrW_messageTitle",
			"savedToggle": "PA7OrW_savedToggle",
			"subTab": "PA7OrW_subTab",
			"delightText": "PA7OrW_delightText",
			"commentComposer": "PA7OrW_commentComposer",
			"cognitionSummary": "PA7OrW_cognitionSummary",
			"profileCard": "PA7OrW_profileCard",
			"barFill": "PA7OrW_barFill",
			"topicBadge": "PA7OrW_topicBadge",
			"delightPlatform": "PA7OrW_delightPlatform",
			"messageCard": "PA7OrW_messageCard",
			"footerItemMeta": "PA7OrW_footerItemMeta",
			"confirmCount": "PA7OrW_confirmCount",
			"body": "PA7OrW_body",
			"confirmPanel": "PA7OrW_confirmPanel",
			"historyRestoreList": "PA7OrW_historyRestoreList",
			"contextBarHead": "PA7OrW_contextBarHead",
			"cognitionToggle": "PA7OrW_cognitionToggle",
			"profilePhaseCopy": "PA7OrW_profilePhaseCopy",
			"cardMeta": "PA7OrW_cardMeta",
			"mbtiDimBarFill": "PA7OrW_mbtiDimBarFill",
			"contextValue": "PA7OrW_contextValue",
			"delightCover": "PA7OrW_delightCover",
			"llmInstanceMain": "PA7OrW_llmInstanceMain",
			"delightKickerLine": "PA7OrW_delightKickerLine",
			"cognitionMeta": "PA7OrW_cognitionMeta",
			"sidebarButton": "PA7OrW_sidebarButton",
			"settingsRow": "PA7OrW_settingsRow",
			"insightEvidence": "PA7OrW_insightEvidence",
			"loadingRow": "PA7OrW_loadingRow",
			"commentRow": "PA7OrW_commentRow",
			"commentInput": "PA7OrW_commentInput",
			"initPromptCopy": "PA7OrW_initPromptCopy",
			"spinner": "PA7OrW_spinner",
			"probe": "PA7OrW_probe",
			"dialogueAction": "PA7OrW_dialogueAction",
			"profilePortraitP": "PA7OrW_profilePortraitP",
			"mbtiContainer": "PA7OrW_mbtiContainer",
			"confirmTitle": "PA7OrW_confirmTitle",
			"watchToggle": "PA7OrW_watchToggle",
			"profileGroup": "PA7OrW_profileGroup",
			"footerItemKind": "PA7OrW_footerItemKind",
			"interestDomainHeader": "PA7OrW_interestDomainHeader",
			"interestTree": "PA7OrW_interestTree",
			"interestDomain": "PA7OrW_interestDomain",
			"drawerPanel": "PA7OrW_drawerPanel",
			"awarenessItem": "PA7OrW_awarenessItem",
			"poolChip": "PA7OrW_poolChip",
			"footerCopy": "PA7OrW_footerCopy",
			"chipWeight": "PA7OrW_chipWeight",
			"delightHeader": "PA7OrW_delightHeader",
			"llmInstanceHead": "PA7OrW_llmInstanceHead",
			"turnUser": "PA7OrW_turnUser",
			"llmInstanceList": "PA7OrW_llmInstanceList",
			"activityFooter": "PA7OrW_activityFooter",
			"chatInput": "PA7OrW_chatInput",
			"footerHistory": "PA7OrW_footerHistory",
			"probeDomain": "PA7OrW_probeDomain",
			"loadMore": "PA7OrW_loadMore",
			"settingsInstance": "PA7OrW_settingsInstance",
			"settingsPanel": "PA7OrW_settingsPanel",
			"iconButton": "PA7OrW_iconButton",
			"footerItemSummary": "PA7OrW_footerItemSummary",
			"delightScore": "PA7OrW_delightScore",
			"stats": "PA7OrW_stats",
			"insightConfidenceBar": "PA7OrW_insightConfidenceBar",
			"messageActions": "PA7OrW_messageActions",
			"pinnedFooter": "PA7OrW_pinnedFooter",
			"llmInstanceName": "PA7OrW_llmInstanceName",
			"dialogOverlay": "PA7OrW_dialogOverlay",
			"updateRow": "PA7OrW_updateRow",
			"interestDomainWeight": "PA7OrW_interestDomainWeight",
			"recHeaderTitle": "PA7OrW_recHeaderTitle",
			"toolbar": "PA7OrW_toolbar",
			"awarenessObservation": "PA7OrW_awarenessObservation",
			"dialogFields": "PA7OrW_dialogFields",
			"recHeader": "PA7OrW_recHeader",
			"delightCard": "PA7OrW_delightCard",
			"pendingObservation": "PA7OrW_pendingObservation",
			"turnStatus": "PA7OrW_turnStatus",
			"delightDismiss": "PA7OrW_delightDismiss",
			"initPrompt": "PA7OrW_initPrompt",
			"recCoverText": "PA7OrW_recCoverText",
			"stateBadge": "PA7OrW_stateBadge",
			"bellBadge": "PA7OrW_bellBadge",
			"savedFeedbackRow": "PA7OrW_savedFeedbackRow",
			"delightRow": "PA7OrW_delightRow",
			"pendingItem": "PA7OrW_pendingItem",
			"settingsInstanceHead": "PA7OrW_settingsInstanceHead",
			"viewKicker": "PA7OrW_viewKicker",
			"syncInlineDetail": "PA7OrW_syncInlineDetail",
			"interestDomainName": "PA7OrW_interestDomainName",
			"turnSoul": "PA7OrW_turnSoul",
			"insightNote": "PA7OrW_insightNote",
			"drawerEmpty": "PA7OrW_drawerEmpty",
			"panel": "PA7OrW_panel",
			"recTitle": "PA7OrW_recTitle",
			"mbtiConfidence": "PA7OrW_mbtiConfidence",
			"activityLine": "PA7OrW_activityLine",
			"action_confirm": "PA7OrW_action_confirm",
			"dialogCard": "PA7OrW_dialogCard",
			"chainBtn": "PA7OrW_chainBtn",
			"badgeRow": "PA7OrW_badgeRow",
			"footerItem": "PA7OrW_footerItem",
			"mbtiTypeLabel": "PA7OrW_mbtiTypeLabel",
			"poolChipLabel": "PA7OrW_poolChipLabel",
			"contextInterpretation": "PA7OrW_contextInterpretation",
			"statusDot": "PA7OrW_statusDot",
			"brandTitle": "PA7OrW_brandTitle",
			"interestTreeLabel": "PA7OrW_interestTreeLabel",
			"delightTitle": "PA7OrW_delightTitle",
			"tab": "PA7OrW_tab",
			"cognitionCard": "PA7OrW_cognitionCard",
			"settingsActions": "PA7OrW_settingsActions",
			"delightNav": "PA7OrW_delightNav",
			"footerHeadline": "PA7OrW_footerHeadline",
			"syncResults": "PA7OrW_syncResults",
			"chainSection": "PA7OrW_chainSection",
			"cognitionContext": "PA7OrW_cognitionContext",
			"chainHead": "PA7OrW_chainHead",
			"awarenessDate": "PA7OrW_awarenessDate",
			"dialogueEvidence": "PA7OrW_dialogueEvidence",
			"brand": "PA7OrW_brand",
			"footerHint": "PA7OrW_footerHint",
			"statusText": "PA7OrW_statusText",
			"pendingInterpretation": "PA7OrW_pendingInterpretation",
			"confirmItem": "PA7OrW_confirmItem",
			"actionButton": "PA7OrW_actionButton",
			"drawerEmptyTitle": "PA7OrW_drawerEmptyTitle"
		};
		//#endregion
		...
		*  time and summary, plus a dashed load-more button. */
		...
		function InitPrompt(props) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				className: panel_module_css_default.initPrompt,
				children: [(0, react_jsx_runtime.jsxs)("div", {
					className: panel_module_css_default.initPromptCopy,
					children: [
						(0, react_jsx_runtime.jsx)("strong", { children: "画像还没攒起来" }),
						(0, react_jsx_runtime.jsx)("span", { children: "先跑一次初始化：拉取你的平台历史、生成画像，并准备首轮推荐池。" }),
						status !== "" ? (0, react_jsx_runtime.jsx)("span", {
							className: panel_module_css_default.initPromptStatus,
							role: "status",
							children: status
						}) : null
					]
				}), (0, react_jsx_runtime.jsx)(ActionButton, {
					label: busy ? "初始化中…" : "开始初始化",
					primary: true,
					disabled: busy,
					onClick: () => void run()
				})]
		...
		function RecommendView(props) {
			...
			const [appendBlocked, setAppendBlocked] = (0, react.useState)(false);
			...
			const appendMoreRef = (0, react.useRef)(async () => void 0);
			...
			(0, react.useEffect)(() => {
				...
				const observer = new IntersectionObserver((entries) => {
					for (const entry of entries) if (entry.isIntersecting) appendMoreRef.current();
				}, {
					root,
			...
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				(0, react_jsx_runtime.jsxs)("div", {
					...
					children: [(0, react_jsx_runtime.jsxs)("div", {
						...
						className: panel_module_css_default.recHeaderActions,
				...
				status?.initialized === false ? (0, react_jsx_runtime.jsx)(InitPrompt, {
					base,
					onDone: () => void reload(),
					onError: setError
				}) : null,
				...
				items !== null && items.length > 0 ? (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [(0, react_jsx_runtime.jsx)(ActionButton, {
						...
						onClick: () => {
							setAppendBlocked(false);
							run("append", () => appendRecommendations(base, { excludedBvids: excludeAll }).then((next) => {
								if (next.length === 0) setExhausted(true);
								return next;
							}));
						}
		...
		*/
		...
		function syncStatusLabel(status) {
			switch (status) {
				...
				case "extension_required": return {
					label: "需要连接插件",
					tone: "warning"
				};
				case "failed": return {
		...
		function syncResultSummary(results) {
			...
			let login = 0;
			let localOnly = 0;
			for (const item of results) if (item.status === "synced" || item.status === "already_synced") success += 1;
		...
		function savedItemActionable(item) {
			...
			if (item.sync_status === "unsupported" && (item.error_code === "local_only_source" || item.error_code === "unsupported_content_type")) return false;
			return true;
		}
		...
		function SavedCard(props) {
			...
			const [otherSaved, setOtherSaved] = (0, react.useState)(false);
			const [otherBusy, setOtherBusy] = (0, react.useState)(false);
			const [feedbackBusy, setFeedbackBusy] = (0, react.useState)("");
			const [feedbackStatus, setFeedbackStatus] = (0, react.useState)("");
			const status = syncStatusLabel(item.sync_status);
			const actionable = savedItemActionable(item);
			(0, react.useEffect)(() => {
				let cancelled = false;
				fetchSavedStatus(base, otherKind, item.item_key).then((saved) => {
					if (!cancelled) setOtherSaved(saved);
				}).catch(() => {
					if (!cancelled) setOtherSaved(false);
				});
				return () => {
					cancelled = true;
				};
			...
			const toggleOther = (0, react.useCallback)(async () => {
				if (otherBusy) return;
				setOtherBusy(true);
				try {
					if (otherSaved) await removeSaved(base, otherKind, item.item_key);
					else await saveItem(base, otherKind, {
						source_platform: item.source_platform !== "" ? item.source_platform : "bilibili",
			...
			const feedback = (0, react.useCallback)(async (type, note = "") => {
			...
			const comment = (0, react.useCallback)(() => {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [(0, react_jsx_runtime.jsx)(Thumb, {
					...
					children: [
						...
						(0, react_jsx_runtime.jsxs)("div", {
							className: panel_module_css_default.syncBadgeRow,
							children: [(0, react_jsx_runtime.jsx)("span", {
								className: panel_module_css_default.syncChip,
								"data-tone": status.tone,
								children: status.label
							}), (0, react_jsx_runtime.jsx)("span", {
								className: panel_module_css_default.syncInlineDetail,
								children: syncResultDetail({
						...
						(0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								actionable ? (0, react_jsx_runtime.jsx)(ActionButton, {
									label: syncLabel,
									disabled: anyBusy,
									onClick: () => onSync(item.item_key)
								}) : null,
								(0, react_jsx_runtime.jsx)(ActionButton, {
									label: "移除",
									danger: true,
						...
						(0, react_jsx_runtime.jsxs)("div", {
							className: panel_module_css_default.savedFeedbackRow,
							children: [
								(0, react_jsx_runtime.jsx)(ActionButton, {
									label: "喜欢",
									disabled: anyBusy,
						...
						feedbackStatus !== "" ? (0, react_jsx_runtime.jsx)("div", {
							className: panel_module_css_default.feedbackStatus,
							"data-tone": feedbackBusy !== "" ? "info" : void 0,
		...
		function SavedList(props) {
			...
			const [syncMessage, setSyncMessage] = (0, react.useState)("");
			const [syncError, setSyncError] = (0, react.useState)("");
			const [syncResults, setSyncResults] = (0, react.useState)(null);
			...
			const pollTask = (0, react.useCallback)(async (taskId) => {
			...
			const runSync = (0, react.useCallback)(async (itemKeys) => {
			...
			const syncButtonLabel = syncing ? "同步中…" : pendingCount > 0 ? `同步到平台（${pendingCount}）` : allSynced ? "已全部同步" : "没有可同步项";
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				error !== "" ? (0, react_jsx_runtime.jsx)(ErrorNote, { text: error }) : null,
				total > 0 || syncing ? (0, react_jsx_runtime.jsxs)("div", {
					className: panel_module_css_default.syncToolbar,
					children: [
						(0, react_jsx_runtime.jsx)(ActionButton, {
						...
						syncMessage !== "" ? (0, react_jsx_runtime.jsx)("span", {
							className: panel_module_css_default.syncMessage,
							children: syncMessage
						}) : null,
						syncError !== "" ? (0, react_jsx_runtime.jsx)("span", {
							className: panel_module_css_default.syncMessage,
							"data-tone": "error",
							role: "alert",
							children: syncError
				...
				syncResults !== null && syncResults.length > 0 ? (0, react_jsx_runtime.jsx)("div", {
					className: panel_module_css_default.syncResults,
					children: syncResults.map((result) => {
						const item = items?.find((row) => row.item_key === result.item_key);
						const status = syncStatusLabel(result.status);
						return (0, react_jsx_runtime.jsxs)("div", {
							className: panel_module_css_default.syncLine,
							children: [
								(0, react_jsx_runtime.jsx)("span", {
									className: panel_module_css_default.syncChip,
									"data-tone": status.tone,
								...
								(0, react_jsx_runtime.jsx)("span", {
									className: panel_module_css_default.syncLineText,
									children: item !== void 0 && item.title !== "" ? item.title : result.item_key
								}),
								(0, react_jsx_runtime.jsx)("span", {
									className: panel_module_css_default.syncLineDetail,
									children: syncResultDetail(result)
								})
							]
						}, result.item_key);
					})
				...
				items !== null && items.length > 0 && visibleItems !== null && visibleItems.length === 0 ? (0, react_jsx_runtime.jsx)(EmptyState, { text: "本地条目已全部同步到 B 站；已同步条目已从侧栏隐藏，数据仍保留在本地。" }) : null,
				visibleItems?.map((item) => (0, react_jsx_runtime.jsx)(SavedCard, {
					base,
					listKind,
					item,
					removing,
		...
		function HistoryRestoreRow(props) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				className: panel_module_css_default.historyRestoreRow,
				children: [(0, react_jsx_runtime.jsxs)("span", {
					className: panel_module_css_default.historyRestoreLabel,
					children: ["已移除 · ", label]
				}), (0, react_jsx_runtime.jsx)(ActionButton, {
					label: busy ? "恢复中…" : "恢复",
					disabled: busy,
		...
		function HistoryRestoreActions(props) {
			...
			return (0, react_jsx_runtime.jsx)("div", {
				className: panel_module_css_default.historyRestoreList,
				children: restorable.map((ctx) => (0, react_jsx_runtime.jsx)(HistoryRestoreRow, {
					base,
					item,
					context: ctx,
					onRestored,
					onError
				}, `${ctx.context}:${ctx.occurred_at}`))
			});
		}
		...
		function HistoryList(props) {
			...
			const restoreContext = (0, react.useCallback)(async () => {
				...
				items?.map((item) => (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [(0, react_jsx_runtime.jsx)(Thumb, {
						...
						children: [
							...
							category === "removed" ? (0, react_jsx_runtime.jsx)(HistoryRestoreActions, {
								base,
								item,
								onRestored: () => void restoreContext(),
								onError: setError
							}) : null,
							(0, react_jsx_runtime.jsx)("div", {
								className: panel_module_css_default.cardActions,
		...
		*/
		...
		function CognitionCard(props) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				className: panel_module_css_default.cognitionCard,
				"data-expanded": open || void 0,
				children: [(0, react_jsx_runtime.jsxs)("button", {
					type: "button",
					className: panel_module_css_default.cognitionToggle,
					disabled: !expandable,
					onClick: () => setOpen((v) => !v),
					children: [
						(0, react_jsx_runtime.jsx)("span", {
							className: panel_module_css_default.cognitionSummary,
							children: item.summary !== "" ? item.summary : "阿B 更新了一条认知"
						}),
						item.context_line !== "" ? (0, react_jsx_runtime.jsx)("span", {
							className: panel_module_css_default.cognitionContext,
							children: item.context_line
						}) : null,
						(0, react_jsx_runtime.jsxs)("span", {
							className: panel_module_css_default.cognitionMeta,
							children: [item.source_label !== "" ? (0, react_jsx_runtime.jsx)("span", {
								className: panel_module_css_default.cognitionSource,
								children: item.source_label
							}) : null, item.created_at !== "" ? (0, react_jsx_runtime.jsx)("span", {
								className: panel_module_css_default.cognitionTime,
								children: formatTime(item.created_at)
							}) : null]
						})
					]
				}), expandable && open ? (0, react_jsx_runtime.jsxs)("div", {
					className: panel_module_css_default.cognitionDetails,
					children: [
						item.impact !== "" ? (0, react_jsx_runtime.jsxs)("div", {
							className: panel_module_css_default.cognitionDetail,
							children: [(0, react_jsx_runtime.jsx)("span", {
								className: panel_module_css_default.cognitionDetailLabel,
								children: "影响"
							}), item.impact]
						}) : null,
						item.reasoning !== "" ? (0, react_jsx_runtime.jsxs)("div", {
							className: panel_module_css_default.cognitionDetail,
							children: [(0, react_jsx_runtime.jsx)("span", {
								className: panel_module_css_default.cognitionDetailLabel,
								children: "推理"
						...
						item.evidence !== "" ? (0, react_jsx_runtime.jsxs)("div", {
							className: panel_module_css_default.cognitionDetail,
							children: [(0, react_jsx_runtime.jsx)("span", {
								className: panel_module_css_default.cognitionDetailLabel,
								children: "依据"
		...
		function ProfileView(props) {
			...
			const [cognition, setCognition] = (0, react.useState)({
				items: [],
				hasMore: false,
				nextCursor: ""
			});
			const [cognitionLoading, setCognitionLoading] = (0, react.useState)(false);
			const [error, setError] = (0, react.useState)("");
			const reload = (0, react.useCallback)(async () => {
				setError("");
				try {
			...
			const loadMoreCognition = (0, react.useCallback)(async () => {
				if (!cognition.hasMore || cognitionLoading) return;
				setCognitionLoading(true);
				try {
					const next = await fetchProfileSummary(base, {
						limit: 5,
						cursor: cognition.nextCursor
					});
					if (next === null) return;
					setCognition((prev) => ({
			...
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				...
				(0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						cognition.items.length > 0 ? (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
							(0, react_jsx_runtime.jsx)(Layer, { label: "阿B 最近新记住了什么" }),
							cognition.items.map((item) => (0, react_jsx_runtime.jsx)(CognitionCard, { item }, `${item.created_at}:${item.summary}`)),
							cognition.hasMore ? (0, react_jsx_runtime.jsx)("div", {
								className: panel_module_css_default.cardActions,
								children: (0, react_jsx_runtime.jsx)(ActionButton, {
									label: "加载更早的认知",
									disabled: cognitionLoading,
									onClick: () => void loadMoreCognition()
								})
							}) : null
						] }) : null,
		...
		*/
		...
		const STAR_COUNT_TTL_MS = 720 * 60 * 1e3;
		...
		*/
		function OpenBiliClawPanel({ closePanel, isDark, onThemeChange }) {
			...
			const [starCount, setStarCount] = (0, react.useState)("");
			...
			(0, react.useEffect)(() => {
				...
				let cancelled = false;
				fetchProjectStats(base).then(({ githubStars }) => {
					if (cancelled || githubStars <= 0) return;
					setStarCount(formatStarCount(githubStars));
					try {
						localStorage.setItem(STAR_COUNT_CACHE_KEY, JSON.stringify({
							n: githubStars,
							t: Date.now()
						}));
					} catch {}
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: panel_module_css_default.iconButton,
								title: "电脑版：在浏览器中打开 Web 版",
								onClick: () => {
									window.open(`${base}/`, "_blank", "noopener");
								},
								children: (0, react_jsx_runtime.jsx)(MonitorIcon, { size: 15 })
							}),
							(0, react_jsx_runtime.jsxs)("button", {
								type: "button",
								className: panel_module_css_default.starButton,
								title: "在 GitHub 给 OpenBiliClaw 点个 Star",
								onClick: () => {
									window.open(STAR_REPO_URL, "_blank", "noopener");
								},
								children: [
									(0, react_jsx_runtime.jsx)(GithubStarIcon, { size: 14 }),
									(0, react_jsx_runtime.jsx)("span", {
										className: panel_module_css_default.starLabel,
										children: "Star"
									}),
									starCount !== "" ? (0, react_jsx_runtime.jsx)("span", {
										className: panel_module_css_default.starCount,
										children: starCount
									}) : null
								]
							}),
		...
		*/
		function mountPanel(controller, theme) {
			let root;
			const dispose = waitForElement(PANEL_COL_SELECTOR, (el) => {
				root = (0, react_dom_client.createRoot)(el);
				root.render((0, react_jsx_runtime.jsx)(OpenBiliClawPanel, {
					closePanel: () => {
		...
		//#endregion
		//#region lib/types/client/sidebarButton.js
		/**
		...
		*/
		function OpenBiliClawSidebarButton({ wide, togglePanel }) {
			return (0, react_jsx_runtime.jsxs)("button", {
				type: "button",
				className: panel_module_css_default.sidebarButton,
				"data-wide": wide || void 0,
				title: "OpenBiliClaw",
				onClick: togglePanel,
				children: [(0, react_jsx_runtime.jsx)("img", {
					className: panel_module_css_default.sidebarButtonIcon,
					src: BRAND_ICON,
					alt: "",
					"aria-hidden": "true"
				}), wide ? (0, react_jsx_runtime.jsx)("span", {
					className: panel_module_css_default.sidebarButtonLabel,
					children: "OpenBiliClaw"
				}) : null]
			});
		}
		...
		*/
		function apply(ctx) {
			const layout = new PanelLayoutController(createPanelLayoutStore());
			const theme = {
