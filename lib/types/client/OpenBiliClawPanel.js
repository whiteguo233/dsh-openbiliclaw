...
import { fetchHealth, fetchProjectStats, readApiBase } from "./api.js";
...
import { MessageIcon, ChatIcon, CollapseIcon, GearIcon, GithubStarIcon, LibraryIcon, MonitorIcon, ProfileIcon, SparkleIcon } from "./icons.js";
 ...
 */
export function OpenBiliClawPanel({ closeDrawer, isDark, onThemeChange }) {
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (raw !== null) {
                ...
                if (Number.isFinite(n) && n > 0)
                    setStarCount(formatStarCount(n));
        ...
        void fetchProjectStats(base).then(({ githubStars }) => {
            if (cancelled || githubStars <= 0)
                return;
            setStarCount(formatStarCount(githubStars));
            try {
                localStorage.setItem(STAR_COUNT_CACHE_KEY, JSON.stringify({ n: githubStars, t: Date.now() }));
            }
            catch { /* ignore */ }
        });
    ...
    return (_jsxs("div", { className: css.panel, "data-dark": dark, children: [_jsxs("div", { className: css.header, children: [_jsxs("div", { className: css.brand, children: [_jsx("img", { className: css.brandMark, src: BRAND_ICON, alt: "", "aria-hidden": "true" }), _jsxs("span", { className: css.brandCopy, children: [_jsx("span", { className: css.brandTitle, children: "OpenBiliClaw" }), _jsxs("span", { className: css.status, title: online ? '后端在线' : '后端离线', children: [_jsx("span", { className: css.statusDot, "data-online": online }), _jsx("span", { className: css.statusText, children: online ? '后端在线' : '后端离线' })] })] })] }), _jsx("button", { type: "button", className: css.iconButton, title: "\u7535\u8111\u7248\uFF1A\u5728\u6D4F\u89C8\u5668\u4E2D\u6253\u5F00 Web \u7248", onClick: () => { window.open(`${base}/`, '_blank', 'noopener'); }, children: _jsx(MonitorIcon, { size: 15 }) }), _jsxs("button", { type: "button", className: css.starButton, title: "\u5728 GitHub \u7ED9 OpenBiliClaw \u70B9\u4E2A Star", onClick: () => { window.open(STAR_REPO_URL, '_blank', 'noopener'); }, children: [_jsx(GithubStarIcon, { size: 14 }), _jsx("span", { className: css.starLabel, children: "Star" }), starCount !== '' ? _jsx("span", { className: css.starCount, children: starCount }) : null] }), _jsxs("button", { type: "button", className: css.iconButton, title: "\u6D88\u606F", onClick: openDrawer, children: [_jsx(MessageIcon, { size: 15 }), messageCount > 0 ? _jsx("span", { className: css.bellBadge, children: messageCount > 99 ? '99+' : messageCount }) : null] }), _jsx("button", { type: "button", className: css.iconButton, title: "\u8BBE\u7F6E", onClick: () => setSettingsOpen(open => !open), children: _jsx(GearIcon, { size: 14 }) }), _jsx("button", { type: "button", className: css.iconButton, title: "\u6536\u8D77", onClick: closeDrawer, children: _jsx(CollapseIcon, { size: 14 }) })] }), settingsOpen ? (_jsx(SettingsOverlay, { base: base, onBaseChange: next => setBase(next), onClose: () => setSettingsOpen(false) })) : null, _jsx("div", { className: css.tabBar, children: TABS.map(item => {
