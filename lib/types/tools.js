...
function int(description, required = false) {
    return required
        ? { type: 'integer', required: true, description }
...
function nonEmpty(value, label) {
    ...
    if (text === '')
        throw new Error(`openbiliclaw: ${label} is required`);
    return text;
}
/**
 ...
 */
export function registerBridgeTools(ctx, bridge) {
    ...
    return [
        ...
        register(defineTool({
            ...
            execute: async (args) => {
                ...
                if (typeof args.sourcePlatform === 'string' && args.sourcePlatform.trim() !== '')
                    argv.push('--source-platform', args.sourcePlatform.trim());
                if (typeof args.excludeItemId === 'string' && args.excludeItemId.trim() !== '')
                    argv.push('--exclude-item-id', args.excludeItemId.trim());
                if (args.refreshIfNeeded === true)
                    argv.push('--refresh-if-needed');
        ...
        register(defineTool({
            ...
            execute: async (args) => {
                ...
                if (typeof args.sourcePlatform === 'string' && args.sourcePlatform.trim() !== '')
                    argv.push('--source-platform', args.sourcePlatform.trim());
                if (typeof args.excludedBvids === 'string' && args.excludedBvids.trim() !== '') {
                    for (const id of args.excludedBvids.split(',').map(s => s.trim()).filter(Boolean))
                        argv.push('--exclude-item-id', id);
                }
        ...
        register(defineTool({
            ...
            execute: async (args) => {
                ...
                if (!Number.isFinite(recommendationId))
                    throw new Error('openbiliclaw: recommendationId is required');
                const feedbackType = nonEmpty(args.feedbackType, 'feedbackType');
                const requestId = nonEmpty(args.requestId, 'requestId');
                const argv = ['--recommendation-id', String(recommendationId), '--feedback-type', feedbackType, '--request-id', requestId];
                if (typeof args.note === 'string' && args.note.trim() !== '')
                    argv.push('--note', args.note.trim());
        ...
        register(defineTool({
            ...
            timeoutMs: bridge.config.timeoutMs,
            execute: async (args) => {
                const bvid = nonEmpty(args.bvid, 'bvid');
                const response = nonEmpty(args.response, 'response');
                const requestId = nonEmpty(args.requestId, 'requestId');
                const argv = ['--bvid', bvid, '--response', response, '--request-id', requestId];
                if (typeof args.title === 'string' && args.title.trim() !== '')
                    argv.push('--title', args.title.trim());
                if (typeof args.message === 'string' && args.message.trim() !== '')
                    argv.push('--message', args.message.trim());
        ...
        register(defineTool({
            ...
            parameters: {
                limit: int('How many feed entries (default 10).', false),
            },
            output: TEXT_OUTPUT,
            timeoutMs: bridge.config.timeoutMs,
            isConcurrencySafe: () => true,
        ...
        register(defineTool({
            ...
            timeoutMs: bridge.config.timeoutMs,
            execute: async (args) => {
                const message = nonEmpty(args.message, 'message');
                const argv = ['--message', message];
                if (typeof args.session === 'string' && args.session.trim() !== '')
                    argv.push('--session', args.session.trim());
        ...
        register(defineTool({
            ...
            timeoutMs: bridge.config.timeoutMs,
            execute: async (args) => {
                const domain = nonEmpty(args.domain, 'domain');
                const response = nonEmpty(args.response, 'response');
                const argv = ['--domain', domain, '--response', response];
                if (typeof args.message === 'string' && args.message.trim() !== '')
                    argv.push('--message', args.message.trim());
        ...
        register(defineTool({
            ...
            timeoutMs: bridge.config.timeoutMs,
            execute: async (args) => {
                const target = nonEmpty(args.target, 'target');
                const op = nonEmpty(args.op, 'op');
                const argv = ['--target', target, '--op', op];
                if (typeof args.value === 'string' && args.value.trim() !== '')
                    argv.push('--value', args.value.trim());
                if (typeof args.parent === 'string' && args.parent.trim() !== '')
                    argv.push('--parent', args.parent.trim());
                if (typeof args.weight === 'string' && args.weight.trim() !== '')
                    argv.push('--weight', args.weight.trim());
        ...
        register(defineTool({
            ...
            execute: async (args) => {
                ...
                const argv = ['--list-kind', listKind, '--source-platform', typeof args.sourcePlatform === 'string' && args.sourcePlatform.trim() !== '' ? args.sourcePlatform.trim() : 'bilibili', '--content-id', itemKey];
                if (typeof args.title === 'string' && args.title.trim() !== '')
                    argv.push('--title', args.title.trim());
                if (typeof args.contentUrl === 'string' && args.contentUrl.trim() !== '')
                    argv.push('--content-url', args.contentUrl.trim());
        ...
        register(defineTool({
            ...
            execute: async (args) => {
                const listKind = nonEmpty(args.listKind, 'listKind');
                const itemKey = nonEmpty(args.itemKey, 'itemKey');
                return bridge.run('remove-saved', ['--list-kind', listKind, '--item-key', itemKey]);
            },
        })),
        register(defineTool({
            ...
            execute: async (args) => {
                const listKind = nonEmpty(args.listKind, 'listKind');
                const limit = typeof args.limit === 'number' ? args.limit : 50;
                return bridge.run('list-saved', ['--list-kind', listKind, '--limit', String(Math.max(1, Math.min(200, Math.floor(limit))))]);
            },
        })),
    ];
}
