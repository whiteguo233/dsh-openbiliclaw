...
插件要求 DSH `0.1.5-rc.1` 或更高版本；插件的 peer ABI 已与该版本的 `dsh-*` 包及 `@deepseek-ai/cordis ^4.0.2` 对齐。请不要在同一个 profile 中混用旧版（`0.0.1` / `0.1.x` 之前）的 DSH 工具包。
...
```yaml
# DSH 0.1.5-rc.1+: 新增配置行必须放在 insert 下
- insert:
    - id: openbiliclaw
